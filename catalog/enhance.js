(() => {
  ...
  .project-card .mv-gallery-next{right:88px}
  ...
  .mv-media-hover{position:fixed;z-index:140;pointer-events:none;opacity:0;visibility:hidden;transform:translateY(5px) scale(.98);transition:.11s opacity,.11s transform;padding:7px;border:1px solid rgba(183,162,255,.48);border-radius:15px;background:var(--solid);box-shadow:0 24px 70px rgba(0,0,0,.52);max-width:min(420px,42vw);contain:layout paint}.mv-media-hover.show{opacity:1;visibility:visible;transform:none}.mv-media-hover img{display:block;max-width:min(400px,40vw);max-height:min(310px,40vh);object-fit:contain;border-radius:10px}.mv-media-hover span{display:block;padding:6px 4px 1px;color:var(--muted);font-size:.7rem;font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  ...
  const hideHover=()=>{clearTimeout(hoverTimer);hoverTimer=0;hover.classList.remove('show');hover.style.removeProperty('left');hover.style.removeProperty('top')};
  ...
  function upgradeAll(scope=document){
    ...
    scope.querySelectorAll?.('[data-live-media-role]').forEach(slot=>{const s=stateFor(infoFor(slot));if(!s)return;touched.add(s);if(slot.dataset.liveMediaRole==='gallery'||!s.primarySlot)s.primarySlot=slot;const loading=slot.querySelector?.('.live-media-loading');if(!liveBridge){if(loading){loading.hidden=false;loading.textContent=slot.dataset.liveMediaRole==='gallery'?'Open in the desktop companion for live source media':'Live media';}return}if(slot.dataset.liveMediaRole==='gallery')ensureGalleryUI(slot);const img=slot.querySelector?.('.live-media-image');if(img)img.classList.add('media-zoomable');loadCached(s);visibilityObserver?.observe(slot)});
    // Catalog link enrichment and view/filter changes replace card nodes. Media state is
    // intentionally retained across those renders, so paint it onto each replacement
    // immediately instead of leaving a correct API result stranded in detached DOM.
    for(const s of touched)applyState(s);
    schedulePrime();
  }
  let galleryVideoControls = null;
  function renderLightbox(){
    ...
    if(isVideo){image.hidden=true;image.src='';video.hidden=false;video.poster=item?.posterUrl||item?.previewUrl||'';if(video.src!==item?.url)video.src=item?.url||'';video.setAttribute('aria-label',item?.alt||`${s.title} video`);galleryVideoControls ||= window.EnderloomVideoControls?.attach(video,{title:`${s.title} video`});}else{galleryVideoControls?.destroy();galleryVideoControls=null;if(!video.hidden){video.pause();video.hidden=true}video.removeAttribute('src');video.load();image.hidden=false;image.src=item?.url||'';image.fetchPriority='high';image.alt=item?.alt||s.title}
    lb('mvGalleryPrev').disabled=s.gallery.length<2;lb('mvGalleryNext').disabled=s.gallery.length<2;lb('mvGalleryCount').textContent=s.gallery.length?`${s.index+1} / ${s.gallery.length} · live${isVideo?' video':''}`:(s.galleryAbsent?'Source reports no gallery':s.sourceGalleryAbsent?'Gallery tab empty · checking project post':(item?'1 / 1 · live':'No live media'));
  ...
  function closeGallery(){galleryVideoControls?.destroy();galleryVideoControls=null;try{const video=lb('mvGalleryVideo');video.pause();video.removeAttribute('src');video.load();video.hidden=true;}catch{}lightbox.hidden=true;current=null;document.body.style.overflow=''}
  ...
  document.addEventListener('pointerover',e=>{const img=e.target.closest?.('.live-media-image.media-zoomable');if(!img||img.hidden||!isHttp(img.src))return;hideHover();hoverTimer=setTimeout(()=>{if(!img.isConnected||img.hidden)return;hover.querySelector('img').src=img.dataset.liveOriginal||img.src;hover.querySelector('span').textContent=img.alt||'Live source image';hover.classList.add('show')},110)},true);
  document.addEventListener('pointermove',e=>{if(!hover.classList.contains('show'))return;const pad=18,w=Math.min(420,window.innerWidth*.42),h=Math.min(330,window.innerHeight*.42);let x=e.clientX+18,y=e.clientY+18;if(x+w>window.innerWidth-pad)x=e.clientX-w-18;if(y+h>window.innerHeight-pad)y=e.clientY-h-18;hover.style.left=`${Math.max(pad,x)}px`;hover.style.top=`${Math.max(pad,y)}px`},true);
  document.addEventListener('pointerout',e=>{if(e.target.closest?.('.live-media-image'))hideHover()},true);
  document.addEventListener('pointerdown',hideHover,true);
  document.addEventListener('scroll',hideHover,true);
  document.addEventListener('catalog:view-changing',hideHover);
  ...
  const observer=new MutationObserver(records=>{let changed=false,removedHoveredMedia=false;for(const r of records){for(const n of r.addedNodes)if(n.nodeType===1){upgradeAll(n);changed=true}for(const n of r.removedNodes)if(n.nodeType===1&&(n.matches?.('.live-media-image')||n.querySelector?.('.live-media-image')))removedHoveredMedia=true}if(removedHoveredMedia)hideHover();if(changed)schedulePrime()});observer.observe(document.body,{childList:true,subtree:true});upgradeAll();
