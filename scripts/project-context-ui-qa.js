...
(async()=>{
 ...
 await target.getByText('Source & licenses',{exact:true}).click();await target.getByText('MIT',{exact:true}).waitFor();const metadata=target.getByRole('region',{name:'Source metadata identity_fixture',exact:true});await metadata.getByText('Declared entrypoints · 2',{exact:true}).click();await metadata.getByText('example.Workshop::init',{exact:true}).waitFor();await metadata.getByText('Manifest fingerprint',{exact:true}).click();await metadata.scrollIntoViewIfNeeded();await page.screenshot({path:path.join(root,'output/playwright/project-provenance.png')});
