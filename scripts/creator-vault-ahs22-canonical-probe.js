...
const research = require('../catalog/creator-vault/research/asianhalfsquat.chunk22-source.json');
...
for (const video of research.videos) {
  for (const rec of video.recommendations) {
    ...
    rows.push({videoId:video.id,name:rec.name,canonicalProjectId:hit?hit.id:null,canonicalName:hit?hit.name:null,existingLinks:hit?(hit.providerLinks||[]).length:0,existingProviders:hit?[...new Set((hit.providerLinks||[]).map(link=>link.provider))]:[],timestampSeconds:rec.timestampSeconds,sectionLabel:rec.sectionLabel});
  }
}
...
if (vault.stats.uniqueProjects !== 554 || vault.stats.recommendations !== 750 || research.videos.length !== 1 || rows.length !== 31) process.exitCode = 2;
