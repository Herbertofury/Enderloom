...
const research = require('../catalog/creator-vault/research/asianhalfsquat.chunk19-source.json');
...
console.log(JSON.stringify({currentStats:vault.stats,videos:research.videos.length,sourceMentions:rows.length,matchedExisting:matched.length,newCandidates:unmatched.length,matchedNames:matched.map(r=>r.name),unmatched:unmatched.map(r=>r.name),rows,relatedLinkedEvidence:research.videos.flatMap(v=>v.relatedLinkedEvidence||[])},null,2));
if (vault.stats.uniqueProjects !== 527 || vault.stats.recommendations !== 689 || research.videos.length !== 2 || rows.length !== 31) process.exitCode = 2;
