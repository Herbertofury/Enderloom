...
(async()=>{
 ...
 const context=await service.request('get_project_context',{provider:'modrinth',projectId:'alpha'});assert.equal(context.targets.find(t=>t.id===first.id).source_evidence.length,4,'Project details must reuse canonical config source evidence');assert.deepEqual(context.targets.find(t=>t.id===first.id).source_evidence.map(row=>row.evidence),report.evidence);
