...
(async()=>{
  ...
  await page.getByText('Evidence & source',{exact:true}).click();await page.getByText('measured',{exact:true}).waitFor();await page.getByRole('button',{name:'Verify source',exact:true}).click();await page.getByText('Source: verified · Analysis: verified',{exact:true}).waitFor();await page.getByRole('button',{name:'Check current inputs',exact:true}).click();await page.getByText('Inputs still match',{exact:true}).waitFor();await page.getByText('Evidence & source',{exact:true}).click();
