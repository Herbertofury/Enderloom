...
if (fs.existsSync(sourcePath) || fs.existsSync(closurePath)) {
  console.error(JSON.stringify({error:'chunk32 pre-production collision gate requires batch32 production files to remain absent'},null,2));
  process.exit(2);
}
const vault = loadCreatorVault(root);
if (vault.stats.recommendations !== 996 || vault.stats.uniqueProjects !== 672) {
  console.error(JSON.stringify({error:'chunk32 pre-production collision gate requires untouched chunk31 baseline',actual:{recommendations:vault.stats.recommendations,uniqueProjects:vault.stats.uniqueProjects}},null,2));
  process.exit(2);
}
