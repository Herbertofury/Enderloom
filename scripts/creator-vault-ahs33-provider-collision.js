...
const candidates = require('../catalog/creator-vault/research/asianhalfsquat.chunk33-provider-candidates.json');
...
if (fs.existsSync(sourcePath) || fs.existsSync(closurePath)) {
  console.error(JSON.stringify({error:'chunk33 pre-production collision gate requires production batch33 files to remain absent'},null,2));
  process.exit(2);
}
const vault = loadCreatorVault(root);
if (vault.stats.recommendations !== 995 || vault.stats.uniqueProjects !== 672) {
  console.error(JSON.stringify({error:'chunk33 pre-production collision gate requires untouched chunk32 baseline',actual:{recommendations:vault.stats.recommendations,uniqueProjects:vault.stats.uniqueProjects}},null,2));
  process.exit(2);
}
...
for (const entry of candidates.entries || []) {
  ...
  for (const link of links || []) {
    ...
    for (const hit of byUrl.get(key) || []) if (hit !== id) collisions.push({candidateId:id,name,url:link[1],existingProjectId:hit});
