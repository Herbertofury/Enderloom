...
(async () => {
  ...
  try {
    ...
    assert(start.releases.some(release => release.id === valid.release_id && release.artifacts.some(artifact => artifact.sha256 === valid.artifact.sha256)), 'Typed release projects the verified immutable artifact');
    ...
    assert(!changed.releases.some(release => release.artifacts.some(artifact => artifact.sha256 === current.artifact.sha256)), 'Edited bytes must not be promoted into an official release');
    ...
    assert.deepEqual(persisted.releases,restored.releases); checks.push('service and CLI expose identical typed releases and FileHash records');
