...
const research = require('../catalog/creator-vault/research/asianhalfsquat.chunk13-source.json');
...
console.log(JSON.stringify({currentStats:vault.stats,videos:research.videos.length,sourceMentions:rows.length,matchedExisting:rows.filter(r=>r.canonicalProjectId).length,newCandidates:rows.filter(r=>!r.canonicalProjectId).length,unmatched:rows.filter(r=>!r.canonicalProjectId).map(r=>r.name),rows,unresolvedLinkedEvidence:research.videos.flatMap(v=>v.unresolvedLinkedEvidence||[])},null,2));
