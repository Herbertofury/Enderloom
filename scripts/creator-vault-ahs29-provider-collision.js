...
const candidates = require('../catalog/creator-vault/research/asianhalfsquat.chunk29-provider-candidates.json');
...
if (fs.existsSync(sourcePath) || fs.existsSync(closurePath)) {
  console.error(JSON.stringify({error:'chunk29 pre-production collision gate requires batch29 production files to remain absent'},null,2));
  process.exit(2);
}
const vault = loadCreatorVault(root);
if (vault.stats.recommendations !== 934 || vault.stats.uniqueProjects !== 650) {
  console.error(JSON.stringify({error:'chunk29 pre-production collision gate requires untouched chunk28 baseline',actual:{recommendations:vault.stats.recommendations,uniqueProjects:vault.stats.uniqueProjects}},null,2));
  process.exit(2);
}
