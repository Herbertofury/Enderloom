...
const cli=path.join(root,'native/target/debug/enderloom.exe'),record=process.argv.includes('--record');
...
(async()=>{
  ...
  if(record){const result=spawnSync('ffprobe',['-v','error','-show_entries','format=duration','-show_entries','stream=codec_name,width,height','-of','json',videos[0].path],{encoding:'utf8',windowsHide:true});assert.equal(result.status,0,result.stderr);const video=JSON.parse(result.stdout);assert(Number(video.format.duration)>90);assert(video.streams.some(s=>s.codec_name==='h264'));}
...
})().catch(async e=>{console.error(e.message);if(e.result)fs.writeFileSync(path.join(out,'release-failure.json'),JSON.stringify(e.result,null,2));if(test)try{await run(['test','finish',test])}catch{}process.exitCode=1});
