...
assert.equal(vault.stats.uniqueProjects, 334, '391 mentions must merge to 334 canonical projects');
...
assert.equal(vault.stats.multiProviderProjects, 165);
assert.equal(vault.stats.providerDestinations, 509);
...
// EnderVerse exact source evidence, including Vanilla+ Episodes 1 and 2.
const ender=vault.creators.find(c=>c.id==='youtube:enderversemc'); assert(ender&&ender.coverage.legacyRecovery==='no-public-corpus-prepopulated'); assert.equal(ender.coverage.indexedVideos,4); assert.equal(ender.coverage.recommendationCount,145); assert.equal(ender.coverage.verifiedProjectHomes,144);
const enderVideos=vault.videos.filter(v=>v.creatorId===ender.id); assert.equal(enderVideos.length,4); assert.equal(enderVideos.reduce((s,v)=>s+v.mods.length,0),145);
...
assert.deepEqual(ep2.mods.find(m=>m.name==='Concurrent Chunk Management Engine').loader,['Fabric']); assert.deepEqual(ep2.mods.find(m=>m.name==='Talking Villager').loader,['Fabric']); assert.deepEqual(ep2.mods.find(m=>m.name==='Chisel and Bits').loader,['Forge','Fabric']);
...
const chisels=vault.projects.find(p=>p.id==='chisels-and-bits'); assert(chisels&&chisels.providerLinks.filter(x=>x.provider==='CurseForge').length===2); assert(chisels.providerLinks.some(x=>x.label==='Fabric')); assert(chisels.providerLinks.some(x=>x.label==='Forge/NeoForge'));
const c2me=vault.projects.find(p=>p.id==='concurrent-chunk-management-engine'); assert(c2me&&c2me.providerLinks.length===3); assert.equal(c2me.providerLinks.filter(x=>x.provider==='Modrinth').length,2); assert(c2me.providerLinks.some(x=>x.provider==='Modrinth'&&x.label==='Fabric')); assert(c2me.providerLinks.some(x=>x.provider==='Modrinth'&&x.label==='NeoForge'));
const moreMobs=vault.projects.find(p=>p.id==='more-mobs'); assert(moreMobs&&moreMobs.providerLinks.length===4); assert(moreMobs.providerLinks.some(x=>x.provider==='Official'));
const modernBeta=vault.projects.find(p=>p.id==='modern-beta'); assert(modernBeta&&modernBeta.providerLinks.some(x=>x.provider==='GitHub'&&/b3spectacled\/modern-beta-fabric/.test(x.url)));
const marbled=vault.projects.find(p=>p.id==='marbled-s-villager-hats'); assert(marbled&&marbled.providerLinks.some(x=>x.provider==='Modrinth')&&marbled.providerLinks.some(x=>x.provider==='CurseForge'));
const wanted=vault.projects.find(p=>p.id==='wanted-kyubion'); assert(wanted&&wanted.providerLinks.some(x=>x.provider==='Planet Minecraft'&&/wanted-6073624/.test(x.url)));
...
const rendered=renderCatalog({id:'creator-vault-qa',name:'Creator Vault QA',items:[],assets:{},documents:[],sources:[]},root); for(const needle of ['youtube:kreksuminecraft','youtube:asianhalfsquat','youtube:enderversemc','youtube:AbkSa8oXDpU','Snowy Spirit',"Pufferfish's Skills",'Carry On','https://modrinth.com/mod/c2me-neoforge','https://www.curseforge.com/minecraft/mc-mods/chisels-bits-for-fabric','Find in Enderloom']) assert(rendered.html.includes(needle),`rendered output missing ${needle}`);
