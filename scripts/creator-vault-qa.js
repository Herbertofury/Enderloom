...
assert.equal(kreksu.coverage.indexedVideos, 2, 'Kreksu chunk 1 video count');
assert.equal(kreksu.coverage.recommendationCount, 20, 'Kreksu chunk 1 recommendation count');
assert.equal(vault.videos.length, 2, 'two fully sourced Kreksu videos in chunk 1');
assert.equal(vault.stats.recommendations, 20, 'twenty fully sourced recommendations');
...
assert(rendered.html.includes('Legionary'), 'renderer must embed source-backed recommendations');
