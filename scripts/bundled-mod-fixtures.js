...
function workshopBundle(version='4.0') {
 const quilt=zip({'quilt.mod.json':JSON.stringify({schema_version:1,quilt_loader:{id:'quilt_core',version:'1.3',group:'org.example',metadata:{name:'Quilt Core'},depends:[{id:'helper',versions:'>=2'}]}})});
 const fabric=zip({'fabric.mod.json':JSON.stringify({schemaVersion:1,id:'bundled_helper',name:'Bundled Helper',version,depends:{identity_fixture:'>=2'},jars:[{file:'nested/quilt.jar'}]}),'nested/quilt.jar':quilt});
 const neo=zip({'META-INF/neoforge.mods.toml':'[[mods]]\nmodId="neo_bundle"\nversion="3.1"\ndisplayName="Bundled Neo Library"\n[[dependencies.neo_bundle]]\nmodId="identity_fixture"\ntype="required"\nversionRange="[2,)"\nside="BOTH"\n'});
 const ghost=zip({'fabric.mod.json':'{"id":"unrelated_example","version":"1"}'});
 const primary={'fabric.mod.json':JSON.stringify({id:'identity_fixture',name:'Identity Workshop',version:'2.0',depends:{helper:'>=2',bundled_helper:'>=4'},jars:[{file:'META-INF/jars/helper.jar'},{file:'../escape.jar'},{file:'missing.jar'},{file:'broken.jar'}]}),
 ...
 return {bytes:zip(primary),sha256:crypto.createHash('sha256').update(fabric).digest('hex')};
}
