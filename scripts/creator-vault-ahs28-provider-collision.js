...
const candidates = require('../catalog/creator-vault/research/asianhalfsquat.chunk28-provider-candidates.json');
...
if (fs.existsSync(sourcePath) || fs.existsSync(closurePath)) {
  console.error(JSON.stringify({error:'chunk28 pre-production collision gate requires batch28 production files to remain absent'},null,2));
  process.exit(2);
}
const vault = loadCreatorVault(root);
if (vault.stats.recommendations !== 833 || vault.stats.uniqueProjects !== 600) {
  console.error(JSON.stringify({error:'chunk28 pre-production collision gate requires untouched chunk27 baseline',actual:{recommendations:vault.stats.recommendations,uniqueProjects:vault.stats.uniqueProjects}},null,2));
  process.exit(2);
}
