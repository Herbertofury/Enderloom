...
(async () => {
  ...
  await app.evaluate(() => { global.creativeQa.reports.unshift({ ...global.creativeQa.reports.find(r => r.id === 'other'), id: 'external', at: 4000, video_error: 'Recorder fixture ended early.' }); });
  await page.getByRole('button', { name: 'Refresh test reports', exact: true }).click();
  await page.waitForFunction(() => document.querySelector('select[aria-label="Test report"]')?.value === 'external');
  await page.getByRole('alert').filter({hasText:'Recorder fixture ended early.'}).waitFor();
