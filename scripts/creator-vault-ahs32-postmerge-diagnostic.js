...
assert.equal(video.mods.length,10,'Feb 23 mention count drift');
...
assert.equal(vault.stats.recommendations,1006,'Chunk 32 recommendation total drift');
assert.equal(vault.stats.uniqueProjects,672,'Chunk 32 must not add canonical projects');
assert.equal(vault.stats.verifiedProjects,670,'Chunk 32 must not change linked canonical-project count');
assert.equal(vault.stats.providerDestinations,1241,'Chunk 32 must not mutate provider destinations');
assert.equal(vault.stats.multiProviderProjects,441,'Chunk 32 must not mutate multi-provider project count');
...
assert.equal(ahsVideos.length,53,'AHS recommendation-bearing video count drift');
assert.equal(ahsMods.length,747,'AHS mention count drift');
assert.equal(ahsLinked.length,747,'AHS linked mention count drift');
const report = {
  phase:'chunk-32-post-merge-diagnostic',stats:vault.stats,unresolved,
