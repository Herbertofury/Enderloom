...
| In-place external libraries | CurseForge profiles | Modrinth profiles | **Accepted.** Detect/connect in place with zero mandatory copying; preserve local alias, notes and organization; reconcile moves/version/loader changes; physical-path junction deduplication; disconnect preserves files. |
...
| Catalog-to-manager bridge | External web discovery | Provider project discovery | **Implemented.** Catalog and manager share the shell; researched provider projects can target an instance, and installed provider identity can reopen research. This remains release-gated against regression. |
