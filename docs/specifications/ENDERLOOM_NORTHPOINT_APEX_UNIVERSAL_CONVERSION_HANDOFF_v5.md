...
- [ ] Final packaged bytecode checks actual JVM invocation mode and constant-pool kind.
- [ ] Constructors are owner-exact and never accepted through inheritance.
- [ ] Packaged access legality is checked after applying exact shipped AT/AW/ClassTweaker effects.
- [ ] Final-field writes and final-superclass inheritance are validated.
- [ ] Class/interface hierarchy drift is validated.
- [ ] invokedynamic/LambdaMetafactory SAMs and implementation MethodHandle kinds are exact-audited.
