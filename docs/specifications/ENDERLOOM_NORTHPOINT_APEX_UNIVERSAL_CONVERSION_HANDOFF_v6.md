...
# CODEX EXECUTION LOCK — sole mission

Resume exactly where the current Enderloom repository, Advent of Ascension conversion state, and verified checkpoints left off; preserve all already-valid work, decisions, fixes, evidence, task state, and accepted requirements, and execute the following mission to completion:
...
This is an implementation task, not a planning task. Do not replace execution with another roadmap, summary, speculative redesign, reduced MVP, or new handoff. Do not substitute documentation polishing for implementation. The first useful action after resolving the current canonical repo/checkpoint is to continue the earliest unfinished implementation step that advances the accepted AoA conversion.
...
Codex must execute only the existing accepted Enderloom/AoA scope and state. It may make any implementation, tooling, test, fixture, performance, build-system, or architecture change required to complete the contract, but must not divert into unrelated features or cosmetic refactors that do not advance a gate below.
...
**Implementation crossing:** once the current Enderloom repo and Create/Convert execution path are resolved, make the smallest coherent backend mutation, run targeted tests, checkpoint, and continue. Do not spend repeated waves rewriting execution instructions.
...
**Do not stop before the real workflow works. Implement until every required gate passes.**
