...
import type { ProjectArtifactGraph, ProjectSourcePreview, ProjectContext, CodeSymbols } from "./artifacts";
...
export const api = {
  ...
  getContentCodeSymbols: (targetKind: string, targetId: string, contentKind: string, fileName: string, archivePath: string, classPath?: string) => call<CodeSymbols>("get_content_code_symbols", { targetKind, targetId, contentKind, fileName, archivePath, classPath }),
  ...
  setInstanceFavorite: (instanceId: string, favorite: boolean) =>
    call<void>("set_instance_favorite", { instanceId, favorite }),
  createInstanceTag: (name: string) =>
    call<InstanceTag>("create_instance_tag", { name }),
  renameInstanceTag: (tagId: string, name: string) =>
    call<InstanceTag>("rename_instance_tag", { tagId, name }),
  deleteInstanceTag: (tagId: string) =>
    call<void>("delete_instance_tag", { tagId }),
  setInstanceTag: (instanceId: string, tagId: string, enabled: boolean) =>
    call<void>("set_instance_tag", { instanceId, tagId, enabled }),
  reorderInstanceTags: (tagIds: string[]) =>
    call<void>("reorder_instance_tags", { tagIds }),
  createInstance: (
    name: string,
    versionId: string,
  ...
  disconnectExternalInstance: (instanceId: string) =>
    call<void>("disconnect_external_instance", { instanceId }),
  ...
  connectInstancesInPlace: (kind: string, root: string, ids: string[]) =>
    call<ConnectionOutcome>("connect_instances_in_place", { kind, root, ids }),
  reconcileExternalInstances: () =>
    call<ExternalReconcileReport>("reconcile_external_instances"),
