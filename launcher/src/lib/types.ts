...
export interface InstanceTagging {
  instance_id: string;
  tag_id: string;
}
...
export type LauncherKind = "atlauncher" | "prism" | "modrinth" | "curseforge";
...
export interface ExternalInstanceChange {
  instance_id: string;
  fields: string[];
}

export interface ExternalReconcileReport {
  checked: number;
  refreshed: number;
  unavailable: string[];
  conflicts: Array<[string, string]>;
  changes: ExternalInstanceChange[];
}
