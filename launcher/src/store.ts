...
interface AppStore {
  ...
  setInstanceFavorite: (instanceId: string, favorite: boolean) => Promise<void>;
  createInstanceTag: (name: string) => Promise<InstanceTag>;
  renameInstanceTag: (id: string, name: string) => Promise<void>;
  deleteInstanceTag: (id: string) => Promise<void>;
  setInstanceTag: (instanceId: string, tagId: string, enabled: boolean) => Promise<void>;
  reorderInstanceTags: (ids: string[]) => Promise<void>;
  createInstance: (
    name: string,
    versionId: string,
    loader?: string | null,
  ...
  return (s: AppStore): Partial<AppStore> => {
    ...
    return {
      ...
      instanceOrganization: {
        ...
        favorites: s.instanceOrganization.favorites.filter((id) => !gone.has(id)),
        taggings: s.instanceOrganization.taggings.filter(
          (tagging) => !gone.has(tagging.instance_id),
        ),
      },
...
export const useStore = create<AppStore>((set) => ({
  ...
  instances: [],
  instanceOrganization: { groups: [], placements: [], favorites: [], tags: [], taggings: [] },
  installedIds: [],
  accounts: [],
  auth: { status: "idle" },
  running: {},
  ...
  init: async () => {
    ...
    if (!listenersBound) {
      ...
      track(await listen<CatalogProjectPayload>("catalog:project", (e) => {
        ...
        toast.success(`Opened ${project.title} from Catalog`, {
          description: target
            ? "Review its compatible version and install it into the selected instance."
            : "Choose or create an instance before installing.",
        });
      }));
  ...
  refreshInstances: async () => {
    // A launcher can update or move a connected profile while Enderloom is open.
    // Reconciliation is deliberately metadata-only and preserves local naming,
    // groups, notes, and launch preferences.
    await api.reconcileExternalInstances().catch(() => null);
  ...
  setInstanceFavorite: async (instanceId, favorite) => {
    await api.setInstanceFavorite(instanceId, favorite);
    set((state) => ({
      instanceOrganization: {
        ...state.instanceOrganization,
        favorites: favorite
          ? [...new Set([...state.instanceOrganization.favorites, instanceId])]
          : state.instanceOrganization.favorites.filter((id) => id !== instanceId),
      },
    }));
  },

  createInstanceTag: async (name) => {
    const tag = await api.createInstanceTag(name);
    set((state) => ({
      instanceOrganization: {
        ...state.instanceOrganization,
        tags: [...state.instanceOrganization.tags, tag],
      },
    }));
    return tag;
  },

  renameInstanceTag: async (id, name) => {
    const tag = await api.renameInstanceTag(id, name);
    set((state) => ({
      instanceOrganization: {
        ...state.instanceOrganization,
        tags: state.instanceOrganization.tags.map((item) => (item.id === id ? tag : item)),
      },
    }));
  },

  deleteInstanceTag: async (id) => {
    await api.deleteInstanceTag(id);
    set((state) => ({
      instanceOrganization: {
        ...state.instanceOrganization,
        tags: state.instanceOrganization.tags.filter((tag) => tag.id !== id),
        taggings: state.instanceOrganization.taggings.filter((tagging) => tagging.tag_id !== id),
      },
    }));
  },

  setInstanceTag: async (instanceId, tagId, enabled) => {
    await api.setInstanceTag(instanceId, tagId, enabled);
    set((state) => {
      const without = state.instanceOrganization.taggings.filter(
        (tagging) => tagging.instance_id !== instanceId || tagging.tag_id !== tagId,
      );
      return {
        instanceOrganization: {
          ...state.instanceOrganization,
          taggings: enabled ? [...without, { instance_id: instanceId, tag_id: tagId }] : without,
        },
      };
    });
  },

  reorderInstanceTags: async (ids) => {
    await api.reorderInstanceTags(ids);
    set((state) => {
      const order = new Map(ids.map((id, index) => [id, index]));
      return {
        instanceOrganization: {
          ...state.instanceOrganization,
          tags: state.instanceOrganization.tags
            .map((tag) => ({ ...tag, sort_order: order.get(tag.id) ?? tag.sort_order }))
            .sort((a, b) => a.sort_order - b.sort_order),
        },
      };
    });
  ...
  deleteInstance: async (id) => {
    const instance = useStore.getState().instances.find((i) => i.id === id);
    if (instance?.external) await api.disconnectExternalInstance(id);
    else await api.deleteInstance(id);
    notifyRemoved(
      instance?.external
        ? instance.name
          ? `Disconnected ${instance.name}`
          : "Instance disconnected"
        : instance?.name
          ? `Deleted ${instance.name}`
          : "Instance deleted",
      instance?.external
        ? "Its launcher profile and every game file were left untouched"
        : "Its files are gone from disk",
    );
  ...
  deleteInstances: async (ids) => {
    ...
    const externalIds = new Set(
      useStore
        .getState()
        .instances.filter((instance) => instance.external && ids.includes(instance.id))
        .map((instance) => instance.id),
    );
    for (const id of ids) {
      try {
        const instance = useStore.getState().instances.find((item) => item.id === id);
        if (instance?.external) await api.disconnectExternalInstance(id);
        else await api.deleteInstance(id);
        removed.push(id);
        set(pruneInstances([id]));
    ...
    if (removed.length > 0) {
      const disconnected = removed.filter((id) => externalIds.has(id)).length;
      notifyRemoved(
        `Removed ${removed.length} ${removed.length === 1 ? "instance" : "instances"}`,
        disconnected > 0
          ? `${disconnected} connected ${disconnected === 1 ? "profile was" : "profiles were"} left untouched`
