...
import { Camera, Play, Square, Video, Terminal, FolderOpen, ChevronRight, Download, FlaskConical, Loader2, CheckCircle2, AlertCircle } from "lucide-react";
...
export function TestingLab({ instanceId, onEvidence }: { instanceId: string; onEvidence: () => void }) {
  ...
  const refresh = async () => { const rows=(await api.getTestingReports()).filter(r => r.instance_id === instanceId);setReports(rows);const id=rows.find(r=>r.id===selected)?.id ?? rows[0]?.id;setReport(id ? await api.getTestingReport(id) : null); };
  useEffect(() => {if(selected) void api.getTestingReport(selected).then(setReport).catch(e=>setError(String(e)));},[selected]);
  useEffect(() => { select(null); setWorld(""); void refresh().catch(e => setError(String(e))); void api.listInstanceWorlds(instanceId).then(setWorlds).catch(() => setWorlds([])); }, [instanceId]);
  ...
  const work = async (action: () => Promise<unknown>) => { setBusy(true); setError(null); try { await action(); await refresh(); } catch(e) { setError(String(e)); } finally { setBusy(false); } };
  ...
  const analyze = () => work(async () => {
    ...
    onEvidence(); toast.success("Test logs and Spark profiles added to this instance’s evidence");
  });
  return <div className="test-lab">
    ...
    <div className="test-setup">
      ...
      <button className="btn-primary" disabled={busy || anyRunning || !instanceId} onClick={() => work(async () => { const r = await api.startTestingSession(instanceId, record, 900, world || null); select(r.id); if (r.error) setError(r.error); })}>{busy ? <Loader2 size={16} className="animate-spin"/> : <Play size={16}/>} Start test session</button>
    </div>
    ...
    {!report ? <div className="test-empty"><Camera size={36}/><h3>Your next test, with evidence.</h3><p>Screenshots of an item working, a playable recording, the exact mod versions, and the commands that got you there.</p><code>enderloom test start "Instance name"</code></div> : <>
      ...
      {!!report.evidence?.length && <section><h3>Spark & log findings</h3><div className="test-metrics">{report.evidence.filter(e => e.kind === "spark").map((e,i) => <div key={i}><span>{e.platform} · {((e.duration_ms ?? 0)/1000).toFixed(0)}s profile</span><strong>{e.tps?.toFixed(2) ?? "—"} <small>TPS</small></strong><p>MSPT p95: {e.mspt_p95?.toFixed(2) ?? "—"} ms</p>{e.threads?.filter(t => /^(Render|Server) thread$/.test(t.name)).map(t => <p key={t.name}>{t.name}: Aether {t.mods.find(m => m.name === "aether")?.percent.toFixed(3) ?? "—"}% exclusive samples</p>)}</div>)}</div>{report.evidence.flatMap(e => e.findings ?? []).map(f => <details key={f.category} className="test-details"><summary>{f.category} · {f.count}</summary>{f.examples.map((e,i) => <p key={i}>{e.text}</p>)}</details>)}<p className="test-footnote">Sample shares include waiting when Spark samples it. They do not measure CPU utilization or the FPS you would gain by removing a mod.</p></section>}
