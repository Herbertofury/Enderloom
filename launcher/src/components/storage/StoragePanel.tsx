    ...
    <div className="pb-6">
      ...
      {offered.length === 0 ? (
        ...
        <>
          <Group
            title="Caches"
            description="Enderloom downloads or rebuilds these the moment it needs them again."
            entries={caches}
            chosen={chosen}
            onToggle={toggle}
            onAll={selectGroup}
      ...
      {report.unresolved && (
        <div className="mt-4 flex gap-2.5 rounded-xl border border-border-soft bg-surface-2/50 px-3.5 py-3 text-[11px] text-content-muted">
          ...
          <span>Enderloom is not offering to remove unused game files. {report.unresolved}</span>
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        description="The files below are removed from disk. Enderloom fetches them again when something needs them."
