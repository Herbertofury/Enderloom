...
export function DataLocations({ heading = true }: { heading?: boolean }) {
  ...
  const pick = async (location: DataLocation) => {
    ...
    try {
      ...
      if (!candidate.usable) {
        toast.error("Enderloom cannot use that folder", { description: candidate.problem ?? undefined });
        return;
      }
            ...
            <span className="block space-y-2">
              ...
              <span className="block">
                {pending.location.exists
                  ...
                  : "Nothing has been written there yet, so this only changes where Enderloom will look."}
              </span>
              {pending.candidate.occupied && (
                <span className="block text-warn">
                  That folder already has files in it. Enderloom will refuse rather than mix them.
                </span>
              )}
