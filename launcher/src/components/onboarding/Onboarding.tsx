...
const TITLES: Record<StepId, { title: string; blurb: string }> = {
  welcome: {
    title: "Welcome to Enderloom",
    blurb: "A fast, native Minecraft launcher. Four short steps and you are playing.",
  },
  storage: {
    title: "Where should the files go",
    blurb:
      "Modpacks and worlds run to tens of gigabytes. Put them on a roomier drive now and Enderloom will not have to move them later.",
  },
  import: {
    title: "Bring your instances over",
    blurb: "Enderloom found another launcher on this machine. Copying leaves the original untouched.",
  },
  account: {
  ...
  java: {
    title: "Java",
    blurb: "Minecraft runs on Java. Enderloom uses what it finds unless you point it somewhere else.",
  },
  memory: {
...
export function Onboarding() {
  ...
  const chooseDataFolder = async () => {
    const chosen = await openFolderDialog({
      directory: true,
      title: "Pick a folder for the Enderloom game files",
    });
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-2.5 rounded-2xl border border-warn/25 bg-warn/[0.07] px-4 py-3.5 text-xs text-warn">
                      ...
                      <span className="leading-relaxed">
                        No Java runtime was found. Enderloom can keep managed Eclipse Temurin
