...
import { ArrowUpRight, Braces, Boxes, FolderOpen, Globe2, Loader2, RefreshCw, Server } from 'lucide-react';
  ...
  return <details className="mx-6 mb-5 rounded-xl border border-border-soft bg-surface/80" onToggle={event => setOpen(event.currentTarget.open)}>
    ...
    {open && <div className="space-y-3 border-t border-border-soft p-4">
      ...
      {targets.map(target => <section key={`${target.kind}:${target.id}`} aria-label={`Used in ${target.name}`} className="rounded-xl border border-border-soft bg-surface-2/50 p-3">
        ...
        {!!target.dependencies.length && <details className="mt-3 border-t border-border-soft pt-3"><summary className="cursor-pointer text-xs text-content-muted">Dependencies & dependents · {target.dependencies.length}</summary><p className="my-2 text-[11px] text-content-faint">Declared by installed mod manifests. Version ranges and sides are shown as declared; runtime compatibility is not yet verified.</p><div className="space-y-2">{target.dependencies.map((dependency,index) => <div key={`${dependency.file_name}:${dependency.mod_id}:${index}`} className="rounded-lg bg-surface p-2.5 text-xs"><p className="font-medium text-content">{dependency.direction === 'dependency' ? `${dependency.owner} → ${dependency.mod_id}` : `${dependency.source_title || dependency.owner} → this mod`}<span className="ml-2 text-content-faint">{dependency.kind}{dependency.alternative_group && ' · one of alternatives'}{dependency.unless != null && ' · conditional'}{!dependency.source_enabled && ' · source disabled'}</span></p><p className="mt-1 text-content-faint">{Array.isArray(dependency.version_range) ? dependency.version_range.join(' or ') : typeof dependency.version_range === 'object' ? JSON.stringify(dependency.version_range) : dependency.version_range || 'Range not declared'} · {dependency.side}</p><p className="mt-1 break-all text-[11px] text-content-faint">{dependency.unless != null && `Unless: ${JSON.stringify(dependency.unless)} · `}{dependency.qualified_id && dependency.qualified_id !== dependency.mod_id && `${dependency.qualified_id} · `}{dependency.file_name} · {dependency.manifest}</p></div>)}</div></details>}
        {!!target.worlds?.length && <details className="mt-3 border-t border-border-soft pt-3"><summary className="flex cursor-pointer items-center gap-2 text-xs text-content-muted"><Globe2 size={14}/>World connections · {target.worlds.length}</summary><p className="my-2 text-[11px] text-content-faint">Observed pack selections and dimension storage. These records do not establish safe mod removal or runtime compatibility.</p><div className="space-y-2">{target.worlds.map(world=><section key={world.folder} aria-label={`World connection ${world.name}`} className="rounded-lg bg-surface p-3"><div className="flex items-center justify-between gap-2"><span className="text-xs font-medium text-content">{world.name}{world.minecraft&&<span className="ml-2 text-content-faint">{world.minecraft}</span>}</span><button type="button" className={button} onClick={()=>void openFolder(world.directory)}><FolderOpen size={13}/>Open world folder</button></div>{world.evidence.map((e,index)=><p key={index} title={e.detail} className="mt-2 break-all text-xs text-content-muted"><span className="text-brand">{e.kind==='dimension_storage'?'Dimension storage':e.kind==='enabled_datapack'?'Enabled data pack':'Disabled data pack'}</span> · {e.mod_id}<span className="mt-0.5 block font-mono text-[10px] text-content-faint">{e.path}</span></p>)}{world.warning&&<p className="mt-2 text-xs text-amber-300">{world.warning}</p>}</section>)}</div></details>}
