...
export function ProjectArtifactIdentity({ provider, projectId }: { provider: SearchProvider; projectId: string }) {
  ...
  useEffect(() => {
    ...
    if (!open) return;
    let live = true;
    void api.getProjectArtifactGraph(provider, projectId).then(value => { if (live) { if (!graphMatches(value)) throw new Error('File identity does not match this project.'); setGraph(value); } }).catch(e => { if (live) setError(String(e)); });
    return () => { live = false; };
  }, [open, provider, projectId]);
  async function verify() {
    const requested = identity;
    setBusy(true); setError('');
    try { const value = await api.verifyProjectArtifacts(provider, projectId); if (latest.current === requested) { if (!graphMatches(value)) throw new Error('File identity does not match this project.'); setGraph(value); } }
    catch (e) { if (latest.current === requested) setError(String(e)); }
    finally { if (latest.current === requested) setBusy(false); }
  }
  ...
  return <details className="relative mx-6 mb-5 rounded-xl border border-border-soft bg-surface/80" onToggle={event => setOpen(event.currentTarget.open)}>
    ...
    {open && <div className="space-y-3 border-t border-border-soft p-4">
      ...
      {!!visible?.releases?.length && <p className="text-xs text-content-faint">{visible.releases.length} recorded {visible.releases.length === 1 ? 'release' : 'releases'} · {new Set(visible.releases.flatMap(release => release.artifacts.map(artifact => artifact.sha256))).size} verified file identities</p>}
