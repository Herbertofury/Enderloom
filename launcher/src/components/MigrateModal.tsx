...
function supportsConnect(source: LauncherSource | null) {
  return source?.kind === "modrinth" || source?.kind === "curseforge";
}
...
export function MigrateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const load = useCallback(async (next: LauncherSource) => {
    setSource(next);
    setMode(supportsConnect(next) ? "connect" : "clone");
    setOutcome(null);
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "connect") {
        if (!supportsConnect(source)) throw new Error("This launcher can only be cloned right now.");
        const result = await api.connectInstancesInPlace(
          source.kind,
          source.root,
          [...selected],
        );
        setOutcome({ ids: result.connected, failed: result.failed, mode });
      } else {
        const result = await api.migrateInstances(source.kind, source.root, [...selected]);
        setOutcome({ ids: result.imported, failed: result.failed, mode });
      }
  ...
  return (
    ...
    >
      <ModalHeader
        id="migrate-title"
        title="Add instances from another launcher"
        subtitle="Connect Modrinth and CurseForge profiles in place, or explicitly clone a separate copy."
        icon={
      ...
      <div className="flex min-h-0 flex-1 flex-col px-5 py-4">
        ...
        {!scanning && source && outcome === null && (
          <>
            ...
            {supportsConnect(source) && (
              <div className="mb-3 grid shrink-0 grid-cols-2 gap-1 rounded-xl border border-border-soft bg-surface-2/60 p-1">
                <button
                  type="button"
                  onClick={() => setMode("connect")}
                  className={cn(
                    "flex items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors",
                    mode === "connect" ? "bg-surface-3 text-content" : "text-content-muted hover:text-content",
                  )}
                >
                  <Link2 className="size-4 shrink-0 text-(--accent)" />
                  <span>
                    <span className="block text-xs font-semibold">Connect in place</span>
                    <span className="block text-[10px] text-content-faint">Recommended · no profile copy</span>
                  </span>
                </button>
                <button
                  type="button"
                  ...
                  className={cn(
                    "flex items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors",
                    mode === "clone" ? "bg-surface-3 text-content" : "text-content-muted hover:text-content",
                  )}
                >
                  <Copy className="size-4 shrink-0" />
                  <span>
                    <span className="block text-xs font-semibold">Clone a copy</span>
                    <span className="block text-[10px] text-content-faint">Independent Enderloom folder</span>
                  </span>
                </button>
              </div>
            )}
            ...
            <div className="mb-2 flex shrink-0 items-center justify-between gap-3">
              <span className="text-[11px] text-content-faint">
                {mode === "connect"
                  ? "Enderloom uses the existing profile folder. Disconnecting never deletes it."
            ...
            <div>
              <div className="font-display text-xl font-semibold text-content">
                {outcome.ids.length === 1
                  ? `1 instance ${outcome.mode === "connect" ? "connected" : "cloned"}`
                  : `${outcome.ids.length} instances ${outcome.mode === "connect" ? "connected" : "cloned"}`}
              </div>
              <p className="mt-1 text-xs text-content-muted">
                {outcome.mode === "connect"
                  ? "Their existing files stay where they are and remain visible to the other launcher."
                  : "The copies are independent. Minecraft downloads on the first launch."}
              </p>
            </div>
        ...
        <ModalFooter className="justify-between">
          <span className="text-xs text-content-faint">
            {outcome
              ? ""
              : mode === "connect"
                ? `${selected.size} selected · 0 profile bytes copied`
                : `${selected.size} selected · ${formatBytes(totalBytes)} to copy`}
          </span>
          <div className="flex items-center gap-2">
            ...
            {!outcome && (
              ...
              >
                ...
                {mode === "connect" ? "Connect" : "Clone"}{" "}
                {selected.size === 1 ? "instance" : `${selected.size} instances`}
              </button>
            )}
          </div>
        </ModalFooter>
