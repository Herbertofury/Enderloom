              ...
              <Row label={server.launch_script}>
                ...
                <span className="text-[12px] text-content-muted">
                  {server.skip_launch_script
                    ? "Bootstrap complete, Enderloom runs its own command"
                    : canRunWithoutScript
                      ? "On, Enderloom runs this instead of its own command"
                      : "Required until the script installs the loader"}
                </span>
              </Row>
              <p className="mt-2 max-w-2xl text-[11px] leading-relaxed text-content-faint">
                Enderloom uses this script only while the downloaded pack still needs to install its
                loader. It ignores installer Java processes and switches future launches to
                Enderloom's own Java command only after it finds complete launch files. Packs whose
