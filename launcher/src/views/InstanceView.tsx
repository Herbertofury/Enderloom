...
export function InstanceView() {
  ...
  const openCatalogResearch = (item: ContentItem) => {
    const source = item.source;
    const query = source?.title ?? item.file_name.replace(/\.(disabled|jar|zip)$/gi, "");
    void window.enderloomLauncher
      ...
      .catch((error) =>
        toast.error(`Could not research ${query}`, { description: String(error) }),
      );
  };
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 px-6 pb-4">
          <div className="flex min-w-0 items-end gap-4">
            ...
            <div className="mt-1.5 flex items-center gap-2 text-[11px] text-white/60">
              ...
              {instance.external && (
                <span className="rounded-md border border-(--accent)/30 bg-black/50 px-2 py-0.5 font-semibold text-(--accent-bright) backdrop-blur">
                  CONNECTED IN PLACE
                </span>
              )}
      ...
      <ConfirmDialog
        ...
        title={instance.external ? `Disconnect ${instance.name}?` : `Delete ${instance.name}?`}
        description={
          instance.external
            ? "Enderloom removes only its connection and local metadata. The launcher profile, worlds, mods, configs and screenshots remain exactly where they are."
            : "The whole instance folder is removed from disk, including its worlds, mods, configs, screenshots and every snapshot taken of it. This cannot be undone."
        }
        confirmLabel={instance.external ? "Disconnect only" : "Delete instance"}
