...
function renderDownloads() {
  ...
  for(const d of items){
    ...
    row.querySelector('strong').textContent=d.filename||'Download'; row.querySelector('small').textContent=d.error || (d.state==='progressing'?(total?`${pct}% · ${formatBytes(rec)} / ${formatBytes(total)}`:`${formatBytes(rec)} downloaded`):d.state)+(d.sha256?' · SHA-256 '+d.sha256.slice(0,12)+(d.hashVerified?' · Verified':''): '');
    ...
    if(d.background){if(['progressing','queued'].includes(d.state))add('Cancel','cancel');else if(d.state!=='completed')add('Retry','retry');if(d.sourcePage)add('Open source','source');}
    else if(['progressing','paused'].includes(d.state)){add(d.state==='paused'?'Resume':'Pause',d.state==='paused'?'resume':'pause');add('Cancel','cancel');}
...
function bind(){
  ...
  $('backgroundDownloadForm').onsubmit=async e=>{e.preventDefault();const submit=e.currentTarget.querySelector('button');submit.disabled=true;try{await cmd('start-background-download',{url:$('backgroundDownloadUrl').value,sha256:$('backgroundDownloadHash').value});$('backgroundDownloadUrl').value='';$('backgroundDownloadHash').value='';}catch(error){toast(error.message||String(error));}finally{submit.disabled=false;}}; $('downloadsToggle').onclick=()=>showDownloads(); $('downloadsFolder').onclick=()=>cmd('downloads-folder').catch(()=>{}); $('downloadList').onclick=e=>{const button=e.target.closest('button[data-action]'),row=button?.closest('.download-row');if(!row)return;const operation=button.dataset.action;void action(operation==='open'?'open-download':operation==='reveal'?'reveal-download':operation==='source'?'download-source':'download-control',{id:row.dataset.id,action:operation}).catch(()=>{});}; $('downloadsClose').onclick=()=>showDownloads(false);
