...
pub fn start_launch(app: AppHandle, selector: String, origin: Origin) {
    tauri::async_runtime::spawn(async move {
        let Some(state) = app.try_state::<AppState>() else {
            let waiting =
                Error::other("Enderloom is still starting. Try launching the instance again.");
...
fn program_path() -> String {
    std::env::current_exe()
        ...
        .unwrap_or_else(|_| "enderloom".to_string())
}
...
fn warn_about(ignored: &[String]) {
    ...
    eprintln!("Run with --help to see what Enderloom accepts.");
}
