...
pub fn redact(text: &str, secrets: &[String]) -> String {
    ...
    carried
        ...
        .map(|line| {
            ...
            // Bearer tokens must be removed before generic key/value handling.
            // Otherwise `Authorization: Bearer secret` treats only `Bearer` as
            // the value and can leave `secret` exposed after the marker is gone.
...
mod tests {
    ...
    #[test]
    fn an_authorization_bearer_header_never_leaves_the_token_behind() {
        let token = "enderloom-qa-token-never-real";
        let cleaned = clean(&format!("Authorization: Bearer {token}"));
