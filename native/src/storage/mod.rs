    ...
    let live = if wants_graph {
        ...
        if instances.is_empty() {
            return Err(crate::error::Error::other(
                "Enderloom will not call anything unused while there are no instances.",
            ));
        }
...
pub fn scan(store: &Store, task: Option<&TaskHandle>) -> Result<StorageReport> {
    ...
    if api_bytes > 0 {
        reclaimable.push(Reclaimable {
            ...
            detail: "Replies from Modrinth and CurseForge. These are also what Enderloom falls back on when it cannot reach either site.".to_string(),
            bytes: api_bytes,
            count: 0,
            tier: Tier::Cache,
            items: Vec::new(),
