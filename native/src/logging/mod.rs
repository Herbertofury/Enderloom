...
pub type LogBatchSink = Arc<dyn Fn(Vec<LogRecord>) + Send + Sync + 'static>;
...
fn spawn_service_bridge(sink: LogBatchSink, mut rx: Receiver<LogRecord>) {
...
/// Initializes the same rotating log, runtime filter, and bounded record buffer
/// used by the Tauri shell, but forwards batches over the Electron JSONL event
