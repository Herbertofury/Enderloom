...
fn finish_import_in_background(app: &AppHandle, prepared: packs::PreparedImport) -> Instance {
    ...
    tokio::spawn(async move {
        ...
        packs::finish_import(&state, prepared).await;
    });
    instance
}
...
pub async fn inspect_pack_file(state: State<'_, AppState>, path: String) -> Result<PackPreview> {
    inspect_pack_file_core(&state, &path).await
}

pub async fn inspect_pack_file_core(state: &AppState, path: &str) -> Result<PackPreview> {
...
pub async fn inspect_packwiz_url(state: State<'_, AppState>, url: String) -> Result<PackPreview> {
    inspect_packwiz_url_core(&state, &url).await
}

pub async fn inspect_packwiz_url_core(state: &AppState, url: &str) -> Result<PackPreview> {
...
pub async fn import_pack_file_ipc(
    state: Arc<AppState>,
    path: String,
    name: Option<String>,
) -> Result<Instance> {
    ...
    let prepared = if path
        ...
        .is_some_and(|value| value.eq_ignore_ascii_case("toml"))
    {
        packs::prepare_packwiz_import_ipc(&state, &path.display().to_string(), name).await?
    } else {
        packs::prepare_import_ipc(&state, &path, name).await?
    };
    let instance = prepared.instance().clone();
    tokio::spawn(async move { packs::finish_import(&state, prepared).await });
    Ok(instance)
}
...
pub async fn import_packwiz_url_ipc(
    state: Arc<AppState>,
    url: String,
    name: Option<String>,
) -> Result<Instance> {
    validate_packwiz_url(&url)?;
    let prepared = packs::prepare_packwiz_import_ipc(&state, &url, name).await?;
    let instance = prepared.instance().clone();
    tokio::spawn(async move { packs::finish_import(&state, prepared).await });
    Ok(instance)
}
...
pub async fn export_instance_pack(
    ...
    export_instance_pack_core(&state, &instance_id, &format, &path).await
}

pub async fn export_instance_pack_core(
    state: &AppState,
    instance_id: &str,
    format: &str,
    path: &str,
...
pub fn pack_export_name(name: String, format: String) -> Result<String> {
    pack_export_name_core(&name, &format)
}

pub fn pack_export_name_core(name: &str, format: &str) -> Result<String> {
    let format = PackFormat::parse(format)?;
