...
pub async fn list_instance_content(
    ...
    list_instance_content_core(&state, &instance_id, &kind, reconcile.unwrap_or(false)).await
}

pub(crate) async fn list_instance_content_core(
    state: &AppState,
    instance_id: &str,
    kind: &str,
    reconcile: bool,
...
pub async fn list_instance_content_bundle(
    ...
    list_instance_content_bundle_core(&state, &instance_id, kinds, reconcile.unwrap_or(false)).await
}

pub(crate) async fn list_instance_content_bundle_core(
    state: &AppState,
    instance_id: &str,
    kinds: Vec<String>,
    reconcile: bool,
...
pub async fn plan_content_update(
    ...
    plan_content_update_core(&state, &instance_id, &kind, &file_name).await
}

pub(crate) async fn plan_content_update_core(
    state: &AppState,
    instance_id: &str,
    kind: &str,
    file_name: &str,
...
pub(crate) async fn apply_content_update_ipc(
    state: &AppState,
    instance_id: &str,
    kind: &str,
    file_name: &str,
    ...
    .await?;
    let written = search::resolve::apply_ipc(
        state,
        &plan,
        provider,
        search::resolve::Target::Instance(instance_id),
    ...
    let installed_name = written
        .into_iter()
        .next()
        .map(|item| item.file_name)
        .unwrap_or_else(|| file_name.to_string());
    if installed_name != file_name {
        state
            .db
            .delete_content_file(instance_id, kind, file_name)
            .ok();
