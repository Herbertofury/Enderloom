...
pub async fn install_java_runtime(
    ...
    install_java_runtime_body(&state, major, instance_id.as_deref(), task).await
}

pub(crate) async fn install_java_runtime_ipc(
    state: &AppState,
    major: u32,
...
async fn install_java_runtime_body(
    state: &AppState,
    major: u32,
    instance_id: Option<&str>,
...
pub async fn reconnect_discord(state: State<'_, AppState>) -> Result<()> {
    reconnect_discord_core(&state).await
}

pub(crate) async fn reconnect_discord_core(state: &AppState) -> Result<()> {
...
pub async fn test_network(state: State<'_, AppState>, url: Option<String>) -> Result<NetworkProbe> {
    test_network_core(&state, url).await
}

pub(crate) async fn test_network_core(
    state: &AppState,
    url: Option<String>,
) -> Result<NetworkProbe> {
...
pub fn reset_launcher(app: AppHandle, state: State<AppState>, deep: bool) -> Result<()> {
    ...
    if running {
        return Err(Error::other("Close the game before resetting Enderloom."));
    }
...
pub fn preview_launch_args(
    ...
    preview_launch_args_core(&state, &settings)
}

pub(crate) fn preview_launch_args_core(
    state: &AppState,
    settings: &LauncherSettings,
) -> Result<launch::LaunchPreview> {
