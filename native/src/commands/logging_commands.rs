...
pub fn set_log_level_core(state: &AppState, level: &str) -> Result<LogConfig> {
...
pub fn set_log_level(state: State<AppState>, level: String) -> Result<LogConfig> {
    set_log_level_core(&state, &level)
}

pub fn list_instance_logs_core(
    state: &AppState,
    instance_id: &str,
    ...
    for crash in [false, true] {
        ...
        for path in entries {
            ...
            let Ok(meta) = state.files.metadata(&path) else {
                continue;
            };
            if !meta.is_file() {
                continue;
...
pub fn list_instance_logs(
    ...
    list_instance_logs_core(&state, &instance_id)
}

pub fn diagnose_instance_core(
    state: &AppState,
    instance_id: &str,
    name: Option<&str>,
    crash: bool,
    ...
    let logs = list_instance_logs_core(state, instance_id)?;
    for wanted_crash in [true, false] {
        if let Some(log) = logs.iter().find(|log| log.crash == wanted_crash) {
            if let Ok(contents) = read_instance_log(state, instance_id, &log.name, wanted_crash) {
...
pub fn diagnose_instance(
    ...
    diagnose_instance_core(
        &state,
        &instance_id,
        name.as_deref(),
        crash.unwrap_or(false),
    )
}

pub fn search_instance_log_core(
    state: &AppState,
    instance_id: &str,
    name: &str,
    crash: bool,
    ...
    let text = read_instance_log(state, instance_id, name, crash)?;
    Ok(search_text(&text, query, min_level, limit))
}
...
pub fn delete_instance_log(
    ...
    delete_instance_log_core(&state, &instance_id, &name, crash)
}

pub fn delete_instance_log_core(
    state: &AppState,
    instance_id: &str,
    name: &str,
    crash: bool,
    ...
    if !state.files.remove_file_if_exists(&path)? {
        return Err(Error::other(format!("log file does not exist: {name}")));
    }
...
pub fn redact_instance_log(
    ...
    redact_instance_log_core(&state, &instance_id, &name, crash)
}

pub fn redact_instance_log_core(
    state: &AppState,
    instance_id: &str,
    name: &str,
    crash: bool,
...
pub fn redact_text(state: State<AppState>, text: String) -> Result<String> {
    Ok(redact_text_core(&state, &text))
}

pub fn redact_text_core(state: &AppState, text: &str) -> String {
    crate::diagnose::redact::redact(text, &known_secrets(state))
}

#[tauri::command]
#[tracing::instrument(skip(state, text), err)]
pub async fn share_log(state: State<'_, AppState>, text: String) -> Result<String> {
    share_log_core(&state, &text).await
}

pub async fn share_log_core(state: &AppState, text: &str) -> Result<String> {
