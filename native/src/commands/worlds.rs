...
pub async fn import_worlds(
    ...
    import_worlds_with_target(Some(&app), &state, instance_id, source_path, candidate_ids).await
}

pub(crate) async fn import_worlds_ipc(
    state: &AppState,
    instance_id: String,
    source_path: String,
    candidate_ids: Vec<String>,
) -> Result<usize> {
    import_worlds_with_target(None, state, instance_id, source_path, candidate_ids).await
}

async fn import_worlds_with_target(
    ...
    let task = Arc::new(match app {
        Some(app) => state.tasks.start(app, TaskKind::WorldImport, spec)?,
        None => state.tasks.start_ipc(TaskKind::WorldImport, spec)?,
    });
    let files = state.files.clone();
    let source = PathBuf::from(source_path);
...
pub async fn delete_instance_world(
    ...
    delete_instance_world_core(&state, &instance_id, &folder_name).await
}

pub(crate) async fn delete_instance_world_core(
    state: &AppState,
    instance_id: &str,
    folder_name: &str,
) -> Result<()> {
