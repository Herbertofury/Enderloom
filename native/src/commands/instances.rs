...
pub fn get_instance_launch_command(state: State<AppState>, instance_id: String) -> Result<String> {
    get_instance_launch_command_core(&state, &instance_id)
}

pub(crate) fn get_instance_launch_command_core(
    state: &AppState,
    instance_id: &str,
) -> Result<String> {
...
#[tauri::command]
pub fn set_instance_favorite(
    state: State<AppState>,
    instance_id: String,
    favorite: bool,
) -> Result<()> {
    state.db.set_instance_favorite(&instance_id, favorite)
}

#[tauri::command]
pub fn create_instance_tag(state: State<AppState>, name: String) -> Result<crate::db::InstanceTag> {
    state.db.create_instance_tag(&name)
}

#[tauri::command]
pub fn rename_instance_tag(
    state: State<AppState>,
    tag_id: String,
    name: String,
) -> Result<crate::db::InstanceTag> {
    state.db.rename_instance_tag(&tag_id, &name)
}

#[tauri::command]
pub fn delete_instance_tag(state: State<AppState>, tag_id: String) -> Result<()> {
    state.db.delete_instance_tag(&tag_id)
}

#[tauri::command]
pub fn set_instance_tag(
    state: State<AppState>,
    instance_id: String,
    tag_id: String,
    enabled: bool,
) -> Result<()> {
    state.db.set_instance_tag(&instance_id, &tag_id, enabled)
}

#[tauri::command]
pub fn reorder_instance_tags(state: State<AppState>, tag_ids: Vec<String>) -> Result<()> {
    state.db.reorder_instance_tags(&tag_ids)
}
...
pub fn create_instance(
    ...
    create_instance_core(&state, name, version_id, loader, loader_version)
}

pub(crate) fn create_instance_core(
    state: &AppState,
    name: String,
    version_id: String,
    loader: Option<String>,
...
pub async fn list_loader_versions(
    ...
    list_loader_versions_core(&state, &loader, &game_version).await
}

pub(crate) async fn list_loader_versions_core(
    state: &AppState,
    loader: &str,
    game_version: &str,
) -> Result<Vec<String>> {
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn update_instance_core(
    state: &AppState,
    instance_id: String,
    name: String,
    min_memory_mb: Option<u32>,
...
pub fn set_instance_launch_tools(
    ...
    set_instance_launch_tools_core(&state, &instance_id, &wrapper, &pre_launch, &post_exit)
}

pub(crate) fn set_instance_launch_tools_core(
    state: &AppState,
    instance_id: &str,
    wrapper: &str,
    pre_launch: &str,
...
pub async fn delete_instance(state: State<'_, AppState>, instance_id: String) -> Result<()> {
    delete_instance_core(&state, &instance_id).await
}

pub(crate) async fn delete_instance_core(state: &AppState, instance_id: &str) -> Result<()> {
    crate::snapshots::ensure_no_pending_restore(state, instance_id)?;
    let instance = find_instance(state, instance_id)?;
    if crate::instance_ops::instance_busy(state, instance_id) {
        return Err(Error::other(
            "Stop the instance and wait for its current task before deleting it.",
        ));
    }
    if state.db.instance_is_external(instance_id)? {
        return disconnect_external_instance_core(state, instance_id).await;
    }

    let live = PathBuf::from(&instance.dir);
    let nonce = uuid::Uuid::new_v4();
    let pending = state
        .paths
        .instances()
        .join(format!(".delete-pending-{instance_id}-{nonce}"));
    let committed = state
        .paths
        .instances()
        .join(format!(".delete-committed-{instance_id}-{nonce}"));
    let moved = state.files.exists(&live)?;
    if moved {
        state.files.rename(&live, &pending)?;
    }

    if let Err(error) = state.db.delete_instance(instance_id) {
        if moved {
            state.files.rename(&pending, &live).map_err(|rollback| {
                Error::other(format!(
                    "deleting instance metadata failed: {error}; restoring its folder also failed: {rollback}. Recoverable data remains at {}",
                    pending.display()
                ))
            })?;
        }
    ...
    if moved {
        state.files.rename(&pending, &committed).map_err(|error| {
            Error::other(format!(
                "instance metadata was removed, but its recoverable folder could not be committed for cleanup: {error}. Data remains at {}",
                pending.display()
            ))
        })?;
    }

    if let Err(error) = crate::snapshots::delete_instance_data(state, instance_id).await {
        tracing::warn!(instance_id, %error, "could not remove deleted instance snapshots");
    }
    media::clear_custom_banner(&state.files, instance_id).await;
    state.media_cache.lock().unwrap().remove(instance_id);
    if moved {
        if let Err(error) = state.files.remove_managed_dir_all_if_exists(&committed) {
            tracing::warn!(
                instance_id,
                path = %committed.display(),
                %error,
                "deleted instance folder is quarantined for startup cleanup"
            );
        }
    }
    tracing::info!("instance deleted through recoverable quarantine");
    Ok(())
}

pub(crate) fn recover_committed_instance_deletions(state: &AppState) -> Result<usize> {
    let mut removed = 0usize;
    for path in state
        .files
        .read_dir(state.paths.instances())
        .unwrap_or_default()
    {
        let committed = path
            .file_name()
            .and_then(|name| name.to_str())
            .is_some_and(|name| name.starts_with(".delete-committed-"));
        if committed && state.files.remove_managed_dir_all_if_exists(&path)? {
            removed += 1;
        }
    }
    Ok(removed)
...
pub(crate) async fn disconnect_external_instance_core(
    state: &AppState,
    instance_id: &str,
) -> Result<()> {
    crate::snapshots::ensure_no_pending_restore(state, instance_id)?;
    if !state.db.instance_is_external(instance_id)? {
        return Err(Error::other(
            "That instance is managed by Enderloom, not connected in place.",
        ));
    }
    ...
    tracing::info!("external instance disconnected without deleting its launcher folder");
    Ok(())
}
...
pub async fn get_instance_media(
    ...
    get_instance_media_core(&state, &instance_id).await
}

pub(crate) async fn get_instance_media_core(
    state: &AppState,
    instance_id: &str,
) -> Result<Option<VersionMedia>> {
    ...
    state
        ...
        .lock()
        .unwrap()
        .insert(instance_id.to_string(), result.clone());
    Ok(result)
}
...
pub async fn backfill_pack_logos(state: State<'_, AppState>) -> Result<Vec<Instance>> {
    backfill_pack_logos_core(&state).await
}

pub(crate) async fn backfill_pack_logos_core(state: &AppState) -> Result<Vec<Instance>> {
...
pub async fn list_versions(
    ...
    list_versions_core(&state, include_snapshots).await
}

pub(crate) async fn list_versions_core(
    state: &AppState,
    include_snapshots: bool,
) -> Result<Vec<VersionEntry>> {
...
pub async fn install_instance(
    ...
    install_instance_body(&state, instance, task).await
}

pub(crate) async fn install_instance_ipc(state: &AppState, instance_id: &str) -> Result<()> {
    let instance = find_instance(state, instance_id)?;
...
async fn install_instance_body(
    state: &AppState,
    instance: Instance,
    ...
    let result = async {
        let launch_id = match (&instance.loader, &instance.launch_version_id) {
            (Some(_), None) => {
                let id = loaders::install_loader(state, &instance, &task).await?;
        ...
        install::install_version(state, &instance.id, &launch_id, &task).await
    }
    .await;
...
pub async fn get_java_status(
    ...
    get_java_status_core(&state, &instance_id).await
}

pub(crate) async fn get_java_status_core(
    state: &AppState,
    instance_id: &str,
) -> Result<JavaStatus> {
    let instance = find_instance(state, instance_id)?;
