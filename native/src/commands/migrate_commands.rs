...
use crate::{
    error::{Error, Result},
    migrate::{
        self, ConnectionOutcome, ExternalReconcileReport, LauncherKind, LauncherSource,
...
#[tauri::command]
#[tracing::instrument(skip(state), err)]
pub async fn connect_instances_in_place(
    state: State<'_, AppState>,
    kind: String,
    root: String,
    ids: Vec<String>,
    ...
    let root = PathBuf::from(root);
    let outcome =
        tokio::task::spawn_blocking(move || migrate::connect(&files, &db, kind, &root, &ids))
            .await
...
#[tauri::command]
#[tracing::instrument(skip(state), err)]
pub fn reconcile_external_instances(state: State<'_, AppState>) -> Result<ExternalReconcileReport> {
    migrate::reconcile_external_instances(&state)
}
