...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            if !unavailable.is_empty() {
                ...
                tauri_plugin_dialog::DialogExt::dialog(app.handle())
                    .message(format!(
                        "Enderloom keeps some data on another drive and cannot reach it right now.\n\n{listed}\n\nMount the drive and start Enderloom again, or edit {} to point these folders somewhere else.",
            ...
            match commands::instances::recover_committed_instance_deletions(&state) {
                Ok(count) if count > 0 => {
                    tracing::info!(count, "cleaned committed instance deletions")
                }
                Err(e) => tracing::warn!(error = %e, "could not clean committed instance deletions"),
                _ => {}
            }
            match commands::servers::recover_committed_server_deletions(&state) {
                Ok(count) if count > 0 => {
                    tracing::info!(count, "cleaned committed server deletions")
                }
                Err(e) => tracing::warn!(error = %e, "could not clean committed server deletions"),
                _ => {}
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::instances::create_instance_tag,
            commands::instances::rename_instance_tag,
            commands::instances::delete_instance_tag,
            commands::instances::set_instance_tag,
            commands::instances::reorder_instance_tags,
            ...
            commands::content_commands::get_server_pack_file,
            ...
            commands::content_commands::list_content_source_index,
            ...
            commands::migrate_commands::connect_instances_in_place,
            commands::migrate_commands::reconcile_external_instances,
            ...
            commands::servers::set_server_launch_script,
            commands::servers::rescan_server,
            commands::servers::get_server_script_memory,
            commands::servers::apply_server_script_memory,
            commands::servers::install_server_pack,
            commands::servers::check_server_pack_update,
            commands::servers::install_server_zip,
            ...
            commands::servers::install_server_content,
            ...
            commands::servers::get_server_launch_command,
            ...
            commands::servers::get_server_disk_usage,
