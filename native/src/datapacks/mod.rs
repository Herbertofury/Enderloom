    ...
    for source in sources {
        ...
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(Error::other(format!(
                "{name} is not a file Enderloom can copy"
            )));
        }
...
pub async fn install_ipc(
    ...
    install_with_target(
        None, state, provider, project_id, instance, world, version_id,
    )
    .await
}

async fn install_with_target(
    ...
    let task = match app {
        ...
        None => state.tasks.start_ipc(TaskKind::DatapackInstall, spec)?,
    };
