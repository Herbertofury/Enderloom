...
impl FileManager {
    ...
    /// Drops the exact directory capability for a relocated top-level root.
    ///
    /// Windows keeps an open directory handle non-shareable for rename. Default
    /// folders are covered by the parent data-root capability, but a custom
    /// data location is itself a capability root and must be released before it
    /// can be moved. Other capabilities remain available during the operation.
    pub fn release_exact_root(&self, path: impl AsRef<Path>) -> bool {
        let path = path.as_ref();
        let mut guard = self.roots.write().unwrap();
        if !guard.iter().any(|(root, _)| root == path) {
            return false;
        }
        let kept = guard
            .iter()
    ...
    /// Reads metadata for an external path after following its final symbolic link.
    ///
    /// This is intentionally separate from `external_symlink_metadata`: callers that
    /// enumerate an untrusted tree should continue to inspect and skip nested links,
    /// while integrations may use this method to accept a user-selected directory
    /// whose top-level path is itself a link or junction.
    pub fn external_metadata(&self, path: impl AsRef<Path>) -> Result<std::fs::Metadata> {
    ...
    for path in paths.extra_roots() {
        match Dir::open_ambient_dir(&path, ambient_authority()) {
            ...
            Err(error) => tracing::warn!(
                path = %path.display(),
                error = %error,
                "skipping a folder Enderloom cannot reach right now"
            ),
        }
    }
