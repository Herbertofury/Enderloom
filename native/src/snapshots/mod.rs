...
pub async fn create(
    ...
    create_with_target(Some(app), state, instance, name, excluded).await
}

pub(crate) async fn create_ipc(
    state: &AppState,
    instance: Instance,
    name: Option<String>,
    excluded: Vec<String>,
) -> Result<SnapshotSummary> {
    create_with_target(None, state, instance, name, excluded).await
}
...
async fn create_with_target(
    ...
    let task = Arc::new(match app {
        Some(app) => state.tasks.start(app, TaskKind::SnapshotCreate, spec)?,
        None => state.tasks.start_ipc(TaskKind::SnapshotCreate, spec)?,
    });
    task.stage("indexing");
    let store = SnapshotStore::from_state(state);
