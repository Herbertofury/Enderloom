...
pub async fn repair(app: &AppHandle, state: &AppState, instance: Instance) -> Result<RepairReport> {
    repair_with_target(Some(app), state, instance).await
}

pub async fn repair_ipc(state: &AppState, instance: Instance) -> Result<RepairReport> {
    repair_with_target(None, state, instance).await
}

async fn repair_with_target(
    ...
    let task = Arc::new(match app {
        Some(app) => state.tasks.start(app, TaskKind::InstanceRepair, spec)?,
        None => state.tasks.start_ipc(TaskKind::InstanceRepair, spec)?,
    });
...
pub async fn duplicate(app: &AppHandle, state: &AppState, source: Instance) -> Result<Instance> {
    duplicate_with_target(Some(app), state, source).await
}

pub async fn duplicate_ipc(state: &AppState, source: Instance) -> Result<Instance> {
    duplicate_with_target(None, state, source).await
}

async fn duplicate_with_target(
    app: Option<&AppHandle>,
    state: &AppState,
    source: Instance,
) -> Result<Instance> {
    ...
    let task = Arc::new(match app {
        Some(app) => state.tasks.start(app, TaskKind::InstanceDuplicate, spec)?,
        None => state.tasks.start_ipc(TaskKind::InstanceDuplicate, spec)?,
    });
