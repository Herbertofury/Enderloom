...
pub(super) fn migrate(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS instance_tag_memberships(
            instance_id TEXT NOT NULL,
            tag_id TEXT NOT NULL,
            PRIMARY KEY (instance_id, tag_id),
            FOREIGN KEY (instance_id) REFERENCES instances(id) ON DELETE CASCADE,
            FOREIGN KEY (tag_id) REFERENCES instance_tags(id) ON DELETE CASCADE
        );
        CREATE INDEX IF NOT EXISTS instance_tag_memberships_tag
            ON instance_tag_memberships(tag_id);
    ...
    for (column, declaration) in [
        ...
        ("external_dir", "TEXT"),
        ("external_real_dir", "TEXT"),
        ("external_root", "TEXT"),
        ("external_seen_at", "INTEGER"),
    ] {
        add_column_if_missing(conn, "instances", column, declaration)?;
    }
    conn.execute_batch(
        "CREATE UNIQUE INDEX IF NOT EXISTS instances_external_real_dir
         ON instances(external_real_dir COLLATE NOCASE)
