...
use super::{
    Db, InstanceGroup, InstanceOrganization, InstancePlacement, InstanceTag, InstanceTagging,
};
...
fn clean_tag_name(name: &str) -> Result<&str> {
    let name = name.trim();
    if name.is_empty() {
        return Err(Error::other("Tag name cannot be empty."));
    }
    if name.chars().count() > 64 {
        return Err(Error::other(
            "Tag names cannot be longer than 64 characters.",
        ));
    }
    Ok(name)
}
...
impl Db {
    pub fn instance_organization(&self) -> Result<InstanceOrganization> {
        ...
        let favorites = {
            let mut statement = conn.prepare(
                "SELECT id FROM instances WHERE favorite = 1 ORDER BY group_order, created_at",
            )?;
        ...
        let tags = {
            let mut statement = conn.prepare(
                "SELECT id, name, sort_order FROM instance_tags ORDER BY sort_order, name",
            )?;
            let rows = statement.query_map([], |row| {
                Ok(InstanceTag {
                    id: row.get(0)?,
        ...
        let taggings = {
            let mut statement = conn.prepare(
                "SELECT membership.instance_id, membership.tag_id
                 FROM instance_tag_memberships membership
                 INNER JOIN instances ON instances.id = membership.instance_id
                 INNER JOIN instance_tags tags ON tags.id = membership.tag_id
                 ORDER BY tags.sort_order, membership.instance_id",
            )?;
            let rows = statement.query_map([], |row| {
                Ok(InstanceTagging {
                    instance_id: row.get(0)?,
    ...
    pub fn set_instance_favorite(&self, instance_id: &str, favorite: bool) -> Result<()> {
        let conn = self.0.lock().unwrap();
        let changed = conn.execute(
            "UPDATE instances SET favorite = ?2 WHERE id = ?1",
            params![instance_id, favorite],
        )?;
        if changed == 0 {
            return Err(Error::NotFound("instance".to_string()));
        }
    ...
    pub fn create_instance_tag(&self, name: &str) -> Result<InstanceTag> {
        let name = clean_tag_name(name)?;
        let id = uuid::Uuid::new_v4().to_string();
        let conn = self.0.lock().unwrap();
        let order: i64 = conn.query_row(
            "SELECT COALESCE(MAX(sort_order), -1) + 1 FROM instance_tags",
            [],
            |row| row.get(0),
        )?;
        conn.execute(
            "INSERT INTO instance_tags (id, name, sort_order) VALUES (?1, ?2, ?3)",
            params![id, name, order],
        )?;
        Ok(InstanceTag {
            id,
    ...
    pub fn rename_instance_tag(&self, id: &str, name: &str) -> Result<InstanceTag> {
        let name = clean_tag_name(name)?;
        let conn = self.0.lock().unwrap();
        let changed = conn.execute(
            "UPDATE instance_tags SET name = ?2 WHERE id = ?1",
            params![id, name],
        )?;
        if changed == 0 {
            return Err(Error::NotFound("instance tag".to_string()));
        }
        let sort_order = conn.query_row(
            "SELECT sort_order FROM instance_tags WHERE id = ?1",
            [id],
            |row| row.get(0),
        )?;
        Ok(InstanceTag {
    ...
    pub fn delete_instance_tag(&self, id: &str) -> Result<()> {
        let mut conn = self.0.lock().unwrap();
        let transaction = conn.transaction()?;
        transaction.execute(
            "DELETE FROM instance_tag_memberships WHERE tag_id = ?1",
            [id],
        )?;
        let changed = transaction.execute("DELETE FROM instance_tags WHERE id = ?1", [id])?;
        if changed == 0 {
            return Err(Error::NotFound("instance tag".to_string()));
        }
        transaction.commit()?;
    ...
    pub fn set_instance_tag(&self, instance_id: &str, tag_id: &str, enabled: bool) -> Result<()> {
        let mut conn = self.0.lock().unwrap();
        let transaction = conn.transaction()?;
        for (table, id, label) in [
            ("instances", instance_id, "instance"),
            ("instance_tags", tag_id, "instance tag"),
        ] {
            let exists = transaction
                .query_row(
                ...
                .is_some();
            if !exists {
                return Err(Error::NotFound(label.to_string()));
            }
        }
        if enabled {
            transaction.execute(
                "INSERT OR IGNORE INTO instance_tag_memberships (instance_id, tag_id)
                 VALUES (?1, ?2)",
                params![instance_id, tag_id],
            )?;
        } else {
            transaction.execute(
                "DELETE FROM instance_tag_memberships WHERE instance_id = ?1 AND tag_id = ?2",
                params![instance_id, tag_id],
            )?;
        }
        transaction.commit()?;
    ...
    pub fn reorder_instance_tags(&self, ids: &[String]) -> Result<()> {
        if ids.iter().collect::<HashSet<_>>().len() != ids.len() {
            return Err(Error::other("Tag order contains duplicates."));
        }
        let mut conn = self.0.lock().unwrap();
        let transaction = conn.transaction()?;
        let count: i64 =
            transaction.query_row("SELECT COUNT(*) FROM instance_tags", [], |row| row.get(0))?;
        if count != ids.len() as i64 {
            return Err(Error::other("Tag order is incomplete."));
        }
        for (order, id) in ids.iter().enumerate() {
            let changed = transaction.execute(
                "UPDATE instance_tags SET sort_order = ?2 WHERE id = ?1",
                params![id, order as i64],
            )?;
            if changed == 0 {
                return Err(Error::NotFound("instance tag".to_string()));
    ...
    pub fn clone_instance_placement(&self, source_id: &str, destination_id: &str) -> Result<()> {
        let mut conn = self.0.lock().unwrap();
        let transaction = conn.transaction()?;
        let (group_id, favorite): (Option<String>, bool) = transaction.query_row(
            "SELECT group_id, favorite FROM instances WHERE id = ?1",
            [source_id],
        ...
        let changed = transaction.execute(
            "UPDATE instances SET group_id = ?2, group_order = ?3, favorite = ?4 WHERE id = ?1",
            params![destination_id, group_id, order, favorite],
        )?;
        if changed == 0 {
            return Err(Error::NotFound("destination instance".to_string()));
        }
        transaction.execute(
            "DELETE FROM instance_tag_memberships WHERE instance_id = ?1",
            [destination_id],
        )?;
        transaction.execute(
            "INSERT INTO instance_tag_memberships (instance_id, tag_id)
             SELECT ?2, tag_id FROM instance_tag_memberships WHERE instance_id = ?1",
...
mod tests {
    ...
    #[test]
    fn duplicated_instances_inherit_their_organization() {
        let db = Db::open_in_memory().unwrap();
        db.insert_instance(&instance("one")).unwrap();
        db.insert_instance(&instance("two")).unwrap();
        let group = db.create_instance_group("Modpacks").unwrap();
        let tag = db.create_instance_tag("Favorites from 1.20.1").unwrap();
        db.move_instance_to_group("one", Some(&group.id)).unwrap();
        db.set_instance_favorite("one", true).unwrap();
        db.set_instance_tag("one", &tag.id, true).unwrap();
        ...
        assert!(organization.favorites.iter().any(|id| id == "two"));
        assert!(organization
            .taggings
            .iter()
            .any(|entry| entry.instance_id == "two" && entry.tag_id == tag.id));
    }

    #[test]
    fn tag_deletion_clears_memberships_without_touching_instances() {
        let db = Db::open_in_memory().unwrap();
        db.insert_instance(&instance("one")).unwrap();
        let tag = db.create_instance_tag("Adventure").unwrap();
        db.set_instance_tag("one", &tag.id, true).unwrap();

        db.delete_instance_tag(&tag.id).unwrap();

        let organization = db.instance_organization().unwrap();
        assert!(organization.tags.is_empty());
        assert!(organization.taggings.is_empty());
        assert_eq!(organization.placements.len(), 1);
    }
}
