...
use serde::Serialize;
use crate::{db::ContentFile, error::{Error, Result}};
use super::Db;
...
mod tests {
    ...
    fn artifact(bytes: &[u8]) -> ArtifactIdentity {
        ...
        ArtifactIdentity { sha256: sha256.clone(), size: bytes.len() as u64, evidence_class: "measured".into(), hashes: vec![
            FileHash { algorithm: "sha256".into(), value: sha256 },
            FileHash { algorithm: "sha1".into(), value: sha1_smol::Sha1::from(bytes).digest().to_string() },
            FileHash { algorithm: "sha512".into(), value: format!("{:x}", Sha512::digest(bytes)) },
        ] }
    }
    fn source(value: &ArtifactIdentity) -> ContentFile {
        ContentFile { file_name: "same-name.jar".into(), provider: Some("curseforge".into()), project_id: Some("project-a".into()), version_id: Some("version-1".into()), title: Some("Exact mod".into()), sha1: Some(value.hashes[1].value.clone()), ..ContentFile::default() }
    }
    #[test]
    fn release_projection_excludes_modified_bytes_and_preserves_immutable_history() {
        let db = Db::open_in_memory().unwrap();
        let original = artifact(b"original"); let changed = artifact(b"modified");
        let source = source(&original);
        for value in [&original, &changed, &original] { db.record_artifact_observation("instance","one","mods",&source,value).unwrap(); }
        let graph = db.project_artifact_graph("curseforge","project-a").unwrap();
        ...
        assert_eq!(graph.observations[0].previous_sha256.as_deref(),Some(changed.sha256.as_str()));
        assert_eq!(graph.observations[1].source_match,"modified");
        db.record_artifact_observation("instance","one","mods",&source,&original).unwrap();
        assert_eq!(db.project_artifact_graph("curseforge","project-a").unwrap().observations.len(),3);
        let mut conflict = original.clone(); conflict.size += 1;
        assert!(db.record_artifact_observation("instance","one","mods",&source,&conflict).is_err());
        assert_eq!(db.project_artifact_graph("curseforge","project-a").unwrap().releases[0].artifacts[0].size,original.size);
        let mut invalid = original.clone(); invalid.hashes[0].value = changed.sha256;
        assert!(db.record_artifact_observation("instance","one","mods",&source,&invalid).is_err());
    }
    #[test]
    fn project_without_release_remains_visible_and_cannot_borrow_another_project() {
        let db = Db::open_in_memory().unwrap();
        let value = artifact(b"installed"); let mut source = source(&value); source.version_id = None;
        db.record_artifact_observation("instance","one","mods",&source,&value).unwrap();
        let graph = db.project_artifact_graph("curseforge","project-a").unwrap();
        assert!(graph.releases.is_empty()); assert_eq!(graph.observations.len(),1); assert_eq!(graph.observations[0].release_id,None);
        source.project_id = Some("project-b".into());
        db.record_artifact_observation("instance","one","mods",&source,&value).unwrap();
        assert!(!db.project_artifact_graph("curseforge","project-a").unwrap().observations[0].current);
        assert!(db.project_artifact_graph("curseforge","project-b").unwrap().observations[0].current);
    }
    #[test]
    fn alias_reuses_project_but_provider_version_ids_remain_distinct() {
        let db = Db::open_in_memory().unwrap(); let value = artifact(b"same exact bytes"); let mut source = source(&value);
        db.record_artifact_observation("instance","one","mods",&source,&value).unwrap();
        let id = db.project_artifact_graph("curseforge","project-a").unwrap().project.unwrap().id;
        db.0.lock().unwrap().execute("INSERT INTO graph_project_aliases VALUES('modrinth','alias',?1)",[&id]).unwrap();
        source.provider = Some("modrinth".into()); source.project_id = Some("alias".into());
        db.record_artifact_observation("instance","two","mods",&source,&value).unwrap();
        let graph = db.project_artifact_graph("modrinth","alias").unwrap();
        assert_eq!(graph.project.unwrap().id,id); assert_eq!(graph.releases.len(),2);
    ...
    fn legacy_observations_migrate_idempotently_and_missing_hash_is_not_a_mismatch() {
        let db = Db::open_in_memory().unwrap(); let value = artifact(b"legacy"); let source = source(&value);
        db.record_artifact_observation("instance","one","mods",&source,&value).unwrap();
        { let conn = db.0.lock().unwrap(); conn.execute("DROP TABLE graph_project_observations",[]).unwrap(); migrate_graph(&conn).unwrap(); migrate_graph(&conn).unwrap(); }
        assert_eq!(db.project_artifact_graph("curseforge","project-a").unwrap().observations.len(),1);
        let partial = ArtifactIdentity { hashes: vec![value.hashes[0].clone()], ..value };
        db.record_artifact_observation("instance","two","mods",&source,&partial).unwrap();
        assert_eq!(db.project_artifact_graph("curseforge","project-a").unwrap().observations[0].source_match,"unverified");
    }
}
#[derive(Debug, Serialize)]
...
fn validate_identity(artifact: &ArtifactIdentity) -> Result<()> {
    let hexadecimal = |value: &str, length| value.len() == length && value.bytes().all(|c| c.is_ascii_digit() || (b'a'..=b'f').contains(&c));
    if !hexadecimal(&artifact.sha256, 64) { return Err(Error::other("Artifact identity requires a lowercase SHA-256 digest.")); }
    let mut algorithms = std::collections::BTreeSet::new();
    for hash in &artifact.hashes {
        let length = match hash.algorithm.as_str() { "sha256" => 64, "sha1" => 40, "sha512" => 128, _ => return Err(Error::other("Unsupported artifact hash algorithm.")) };
        if !algorithms.insert(&hash.algorithm) || !hexadecimal(&hash.value, length) {
            return Err(Error::other("Artifact hashes must be unique, valid hexadecimal digests."));
        }
    }
    if !artifact.hashes.iter().any(|hash| hash.algorithm == "sha256" && hash.value == artifact.sha256) {
        return Err(Error::other("Artifact SHA-256 must match its FileHash identity."));
    }
    Ok(())
}
...
fn read_artifact(row: &rusqlite::Row<'_>, offset: usize) -> rusqlite::Result<ArtifactIdentity> {
    ...
    let mut hashes = vec![FileHash { algorithm: "sha256".into(), value: sha256.clone() }];
    for (column, algorithm) in [(offset + 2, "sha1"), (offset + 3, "sha512")] {
        let value: String = row.get(column)?;
        if !value.is_empty() { hashes.push(FileHash { algorithm: algorithm.into(), value }); }
    }
    Ok(ArtifactIdentity { sha256, size: row.get(offset+1)?, hashes, evidence_class: "measured".into() })
}
...
impl Db {
    pub fn record_artifact_observation(&self, target_kind: &str, target_id: &str, kind: &str, source: &ContentFile, artifact: &ArtifactIdentity) -> Result<()> {
        ...
        let stored: (u64,String,String) = tx.query_row("SELECT size,sha1,sha512 FROM graph_artifacts WHERE sha256=?1", [&artifact.sha256], |row| Ok((row.get(0)?,row.get(1)?,row.get(2)?)))?;
        if stored.0 != artifact.size || [(&stored.1,hash("sha1")),(&stored.2,hash("sha512"))].iter().any(|(old,new)| !old.is_empty() && !new.is_empty() && old.as_str()!=*new) {
        ...
        let missing = expected.iter().any(|(expected,actual)| expected.is_some_and(|value| !value.is_empty()) && actual.is_empty());
        let checks: Vec<bool> = expected.iter().filter(|(_,actual)| !actual.is_empty()).filter_map(|(expected,actual)| expected.filter(|value| !value.is_empty()).map(|value| value.eq_ignore_ascii_case(actual))).collect();
        let source_match = if checks.iter().any(|matched| !matched) { "modified" } else if checks.is_empty() || missing { "unverified" } else { "verified" };
        ...
        if previous.as_ref().is_some_and(|(sha,release,matched,project)| sha==&artifact.sha256 && release==&release_id && matched==source_match && project==&canonical_project) { tx.commit()?; return Ok(()); }
        ...
        if let Some(project) = &canonical_project { tx.execute("INSERT INTO graph_project_observations VALUES(?1,?2)",params![project,observation])?; }
    ...
    pub fn project_artifact_graph(&self, provider: &str, project: &str) -> Result<ProjectGraph> {
        ...
        let Some((id,title)) = identity else { return Ok(ProjectGraph {schema_version:1,project:None,releases:vec![],observations:vec![]}); };
        ...
        Ok(ProjectGraph{schema_version:1,project:Some(ProjectIdentity{id,title,aliases}),releases,observations})
    }
}
