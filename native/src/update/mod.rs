...
pub const REPO: &str = "Herbertofury/Enderloom";
pub const REPO_URL: &str = "https://github.com/Herbertofury/Enderloom";
...
fn linux_install_source(
    ...
    if appimage {
        return source(
            ...
            "Enderloom downloads and installs signed updates.",
        );
    }
    if flatpak {
        return source(
...
pub fn install_source() -> InstallSource {
    #[cfg(target_os = "linux")]
    {
        ...
        linux_install_source(
            ...
            nix_store,
            std::path::Path::new("/usr/share/enderloom/aur-package").is_file(),
            std::path::Path::new("/usr/share/enderloom/eopkg-package").is_file(),
        )
    }
...
pub fn electron_install_source(packaged: bool) -> InstallSource {
    if packaged {
        source(
            "electron-release",
            "Enderloom Electron release",
            UpdatePolicy::Manual,
            "Open the Enderloom release page to install a signed Electron update. In-app installation stays disabled until the Electron signing and release manifest contract is published.",
        )
    } else {
        source(
            "electron-source",
            "Enderloom source build",
            UpdatePolicy::Manual,
            "Pull or download a newer Enderloom source checkpoint, then rebuild the Electron application.",
        )
    }
}
...
fn up_to_date(current: String, install_source: InstallSource) -> UpdateInfo {
    UpdateInfo {
        current,
        latest: None,
        notes_url: None,
...
pub async fn check(client: &NetworkManager) -> Result<UpdateInfo> {
    ...
    check_for(client, current, development, install_source()).await
}
...
pub async fn check_for(
    ...
    if response.status() == reqwest::StatusCode::NOT_FOUND {
        ...
        return Ok(up_to_date(current, install_source));
    }
...
pub fn emit_status_ipc(event_sink: &EventSink, status: &AppUpdateStatus) {
    match serde_json::to_value(status) {
        Ok(payload) => event_sink("app:update-status", payload),
        Err(error) => tracing::warn!(%error, "could not serialize app update status"),
    }
}
...
pub async fn check_and_record_ipc(
    client: &NetworkManager,
    coordinator: &UpdateCoordinator,
    current: String,
    packaged: bool,
    ...
    let development = current.contains("-dev.");
    match check_for(
        client,
        current,
        development,
        electron_install_source(packaged),
    )
    .await
    {
        Ok(info) => {
            let status = coordinator.finish_check(Some(info.clone()))?;
            emit_status_ipc(event_sink, &status);
            Ok(info)
        }
        Err(error) => {
            coordinator.fail_check();
...
pub fn dismiss_ipc(
    ...
    let status = coordinator.dismiss(version)?;
    emit_status_ipc(event_sink, &status);
    Ok(status)
}

pub fn download_electron(coordinator: &UpdateCoordinator) -> Result<AppUpdateStatus> {
    let info = coordinator.begin_download()?;
    let status = coordinator.download_failed();
    Err(Error::other(format!(
        "{} Current status returned to {:?}.",
        info.install_source.update_hint, status.phase
    )))
}

pub fn install_electron(_coordinator: &UpdateCoordinator) -> Result<()> {
    Err(Error::other(
        "No verified Electron update is ready. Enderloom will not hand an Electron build to the Tauri installer.",
    ))
}
...
pub async fn download(app: AppHandle, state: &crate::state::AppState) -> Result<AppUpdateStatus> {
    ...
    let task = state.tasks.start(
        ...
        crate::tasks::TaskSpec {
            title: format!("Enderloom {}", info.latest.as_deref().unwrap_or("update")),
...
pub fn install_ready(app: AppHandle, state: &crate::state::AppState) -> Result<()> {
    ...
    if game_running {
        return Err(Error::other("Close the game before restarting Enderloom."));
    }
    if state
        .tasks
        .list()
