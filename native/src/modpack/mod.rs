...
pub use upgrade::{
    check_modpack_upgrade, link_modpack, link_modpack_ipc, plan_modpack_upgrade,
    plan_modpack_upgrade_ipc, recover_interrupted_upgrades, update_between, upgrade_modpack,
    upgrade_modpack_ipc, ModpackUpgrade, ModpackUpgradePlan,
};
...
fn unavailable_curseforge_files(files: &[String]) -> Error {
    ...
    Error::other(format!(
        "CurseForge did not provide downloads for {} pack file{}. Their authors may have disabled third-party downloads, so Enderloom cannot install this pack completely. Use the CurseForge app or choose another pack. Affected: {shown}{suffix}.",
        files.len(),
...
pub(super) async fn prepare_pack_in(
    downloads_dir: &Path,
    state: &AppState,
    provider: Provider,
    project_id: &str,
...
pub async fn plan_modpack_install_ipc(
    downloads_dir: &Path,
    state: &AppState,
    provider: Provider,
    project_id: &str,
...
async fn plan_modpack_install_in(
    downloads_dir: &Path,
    state: &AppState,
    provider: Provider,
    project_id: &str,
...
pub async fn find_manual_download(
    ...
    let downloads_dir = app.path().download_dir()?;
    find_manual_download_in(&downloads_dir, requirement, started_at_ms).await
}

pub async fn find_manual_download_in(
    downloads_dir: &Path,
    requirement: &ManualDownload,
    started_at_ms: u64,
) -> Result<Option<String>> {
...
pub async fn install_modpack_ipc(
    downloads_dir: &Path,
    state: &AppState,
    provider: Provider,
    project_id: &str,
...
async fn install_modpack_in(
    ...
    let task = match app {
        Some(app) => state
            .tasks
            .start(app, crate::tasks::TaskKind::ModpackInstall, spec)?,
        None => state
            .tasks
            .start_ipc(crate::tasks::TaskKind::ModpackInstall, spec)?,
    };
...
pub(crate) async fn install_pack_body(
    ...
    let launch_id = if instance.loader.is_some() {
        let launch_id = loaders::install_loader(state, instance, task).await?;
        launch_id
    } else {
