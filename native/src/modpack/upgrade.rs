...
use super::{
    install_pack_body, loader_from_dependencies, prepare_pack_in, sanitize_relative,
    ManualDownload, ManualDownloadSource, MrIndex, PreparePackOutcome,
};
...
async fn ensure_pack_state_in(downloads_dir: &Path, state: &AppState, instance: &Instance) {
    ...
    if let Err(error) =
        adopt_pack_state_in(downloads_dir, state, instance, provider, project, version).await
    {
        tracing::warn!(
...
async fn adopt_pack_state_in(
    ...
    let prepared = match prepare_pack_in(
        downloads_dir,
        state,
        provider,
        project_id,
    ...
    .await?
    {
        ...
        PreparePackOutcome::NeedsDownloads(_) => {
            return Err(Error::other(
                "This pack version needs files that CurseForge will not hand over automatically, so Enderloom cannot read its baseline.",
            ))
        }
    };
...
pub async fn plan_modpack_upgrade_ipc(
    downloads_dir: &Path,
    state: &AppState,
    instance: &Instance,
    target_version_id: &str,
...
async fn plan_modpack_upgrade_in(
    downloads_dir: &Path,
    state: &AppState,
    instance: &Instance,
    target_version_id: &str,
    ...
    match outcome {
        ...
        PreparePackOutcome::Ready(prepared) => {
            ensure_pack_state_in(downloads_dir, state, instance).await;
...
pub async fn upgrade_modpack_ipc(
    downloads_dir: &Path,
    state: &AppState,
    instance: Instance,
    target_version_id: &str,
...
async fn upgrade_modpack_in(
    ...
    let task = Arc::new(match app {
        Some(app) => state.tasks.start(app, TaskKind::ModpackUpgrade, spec)?,
        None => state.tasks.start_ipc(TaskKind::ModpackUpgrade, spec)?,
    });
    let result = async {
        if snapshot_first {
