...
pub async fn prepare_import(
    ...
    prepare_import_inner(Some(app), false, state, path, name_override).await
}

pub async fn prepare_import_ipc(
    state: &AppState,
    path: &Path,
    name_override: Option<String>,
) -> Result<PreparedImport> {
    prepare_import_inner(None, true, state, path, name_override).await
}
...
pub async fn prepare_packwiz_import(
    ...
    prepare_packwiz_import_inner(Some(app), false, state, source, name_override).await
}

pub async fn prepare_packwiz_import_ipc(
    state: &AppState,
    source: &str,
    name_override: Option<String>,
) -> Result<PreparedImport> {
    prepare_packwiz_import_inner(None, true, state, source, name_override).await
}

async fn prepare_packwiz_import_inner(
    app: Option<&AppHandle>,
    ipc: bool,
    state: &AppState,
    source: &str,
...
async fn prepare_resolved_import(
    ...
    let task = match app {
        Some(app) => state.tasks.start(app, TaskKind::ModpackInstall, spec)?,
        None if ipc => state.tasks.start_ipc(TaskKind::ModpackInstall, spec)?,
        None => return Err(Error::other("modpack import has no task event target")),
    };
...
pub async fn finish_import(state: &AppState, prepared: PreparedImport) {
    let PreparedImport {
        instance,
        instance_dir,
        staged_archive,
