...
fn write_pack(
    ...
    let parent = destination
        .parent()
        .ok_or_else(|| Error::other("The export destination has no parent folder."))?;
    if !parent.as_os_str().is_empty() {
        std::fs::create_dir_all(parent)?;
    }
    let staging = parent.join(format!(".enderloom-export-{}.part", uuid::Uuid::new_v4()));
    ...
    let backup = parent.join(format!(".enderloom-export-{}.backup", uuid::Uuid::new_v4()));
    let had_destination = destination.exists();
    if had_destination {
        if destination.is_dir() {
            let _ = std::fs::remove_file(&staging);
            return Err(Error::other("The export destination is a folder."));
        }
    ...
    if had_destination {
        if let Err(error) = std::fs::remove_file(&backup) {
            tracing::warn!(path = %backup.display(), %error, "old pack export kept as a recovery backup");
        }
    }
