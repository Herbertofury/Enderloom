...
use super::{
    candidate_roots, relative_within, walk_files, ConnectionOutcome, ExternalInstanceState,
    LauncherKind, LauncherSource, MigrationCandidate, MigrationOutcome, MigrationScan,
};
...
pub fn scan(files: &FileManager, root: &Path) -> Result<MigrationScan> {
    ...
    let mut candidates: Vec<MigrationCandidate> = read_profiles(root)?
        ...
        .map(|profile| -> Result<MigrationCandidate> {
            ...
            let present = files
                // A Modrinth profile directory may deliberately be a junction or
                // symlink into a shared CurseForge library. Follow that one boundary;
                // `walk_files` still refuses nested links while enumerating content.
...
pub fn external_path(files: &FileManager, root: &Path, id: &str) -> Result<PathBuf> {
    let profiles = profiles_dir(root);
    let profile = read_profiles(root)?
        .into_iter()
        .find(|profile| profile.id == id)
        .ok_or_else(|| Error::other(format!("unknown Modrinth instance {id}")))?;
    let source = profiles.join(profile.path);
    if relative_within(&profiles, &source).is_none()
        || !files
            .external_metadata(&source)
            .is_ok_and(|metadata| metadata.is_dir())
    {
        return Err(Error::other(format!("unavailable Modrinth instance {id}")));
    }
    Ok(source)
}
...
pub fn external_state(files: &FileManager, root: &Path, id: &str) -> Result<ExternalInstanceState> {
    ...
    Ok(ExternalInstanceState {
        source_dir,
        real_dir,
        version_id: profile.game_version,
        ...
        pack_provider: profile
            ...
            .as_ref()
            .map(|_| "modrinth".to_string()),
        pack_project_id: profile.pack_project,
        pack_version_id: profile.pack_version,
...
pub fn connect(
    ...
    let known_profiles = read_profiles(root)?;
    let mut known_real_dirs = db.external_instance_real_dirs()?;
    ...
    for source_id in ids {
        let result = (|| -> Result<String> {
            let profile = known_profiles
                ...
                .ok_or_else(|| Error::other(format!("unknown Modrinth instance {source_id}")))?;
            let source = external_path(files, root, source_id)?;
            let real = files.canonicalize_external(&source)?;
            if known_real_dirs.iter().any(|path| same_path(path, &real)) {
                return Err(Error::other(
                    "That physical instance is already connected, possibly through a CurseForge junction.",
                ));
            }
            if profile.game_version.is_empty() {
