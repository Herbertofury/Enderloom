...
const RUN_MARKER_PREFIX: &str = "-Denderloom.running_id=";
const LEGACY_RUN_MARKER_PREFIX: &str = "-Dbasalt.running_id=";
...
fn command_has_marker(command: &[std::ffi::OsString], running_id: &str) -> bool {
    ...
    let legacy = format!("{LEGACY_RUN_MARKER_PREFIX}{running_id}");
    command
        .iter()
        .any(|argument| argument == expected.as_str() || argument == legacy.as_str())
}
...
mod tests {
    ...
    fn marker_matches_only_the_expected_run() {
        ...
        assert!(command_has_marker(
            &[OsString::from("-Dbasalt.running_id=run-1")],
            "run-1"
        ));
    }
