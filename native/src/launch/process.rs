    ...
    pub fn emit<T: Serialize>(&self, event: &str, payload: &T) {
        match self {
            ...
            Self::Ipc(sink) => match serde_json::to_value(payload) {
                ...
                Err(error) => tracing::warn!(%error, event, "could not serialize process event"),
            },
        }
    }
}
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            if let Ok(tail) = files.read_tail_async(&path, offset).await {
                ...
                push_log_lines(events.clone(), &running_id, stream, lines, &logs);
            }
...
pub(crate) fn spawn_process_with_events(
    events: ProcessEvents,
...
pub fn recover_processes(
    ...
    recover_processes_with_events(ProcessEvents::tauri(app), registry, files, db, presence)
}

pub fn recover_processes_ipc(
    ...
    recover_processes_with_events(
        ProcessEvents::ipc(event_sink),
        registry,
        files,
        db,
        presence,
...
fn recover_processes_with_events(
    events: ProcessEvents,
    registry: &Arc<Mutex<HashMap<String, RunningHandle>>>,
    files: &FileManager,
    db: &Db,
    ...
    for run in db.active_runs()? {
        ...
        monitor_recovered_process(events.clone(), db.clone(), presence.clone(), run, status);
        recovered += 1;
    }
    if recovered > 0 {
...
mod tests {
    ...
    use super::*;
    use crate::{commands::instances, files::FileManager, paths::Paths, state::AppState};

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn ipc_process_supervision_streams_logs_and_kills_the_verified_child() {
        let root = std::env::temp_dir().join(format!(
            "enderloom-process-ipc-test-{}",
            uuid::Uuid::new_v4()
        ));
        ...
        let instance = instances::create_instance_core(
            &state,
            "Process IPC test".to_string(),
            "1.20.1".to_string(),
            None,
            None,
        )
        ...
        let marker = super::super::identity::run_marker(&running_id);
        ...
        let args = vec![
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("..")
                .join("scripts")
                .join("launcher-process-probe.js")
                .display()
                .to_string(),
            marker,
        ];

        let (event_tx, event_rx) = mpsc::channel::<(String, Value)>();
        let sink: EventSink = Arc::new(move |event, payload| {
            let _ = event_tx.send((event.to_string(), payload));
        });
        let started_at = chrono::Utc::now().timestamp();
        spawn_process_with_events(
            ProcessEvents::ipc(sink),
            &state.running,
            state.files.clone(),
            state.db.clone(),
            ...
            ProcessLaunch {
                ...
                program,
                args,
                cwd: std::path::Path::new(&instance.dir),
                env: Vec::new(),
                post_exit: None,
            },
        )
        ...
        let running_event = event_rx.recv_timeout(Duration::from_secs(5)).unwrap();
        assert_eq!(running_event.0, "process:state");
        assert_eq!(running_event.1["running_id"], running_id);
        assert_eq!(running_event.1["state"], "running");
        assert_eq!(state.db.active_runs().unwrap().len(), 1);
        ...
        for _ in 0..30 {
            {
                ...
                let logs = registry
                    ...
                    .unwrap()
                    .clone();
                found_stdout |= logs.iter().any(|line| {
                    line.stream == "stdout" && line.line.contains("enderloom-ipc-stdout")
                });
                found_stderr |= logs.iter().any(|line| {
                    line.stream == "stderr" && line.line.contains("enderloom-ipc-stderr")
                });
            }
        ...
        assert!(
            found_stdout,
            "stdout was not tailed into the IPC log buffer"
        );
        assert!(
            found_stderr,
            "stderr was not tailed into the IPC log buffer"
        );
        ...
        assert!(state
            ...
            .get_mut(&running_id)
            .unwrap()
            .request_kill(&running_id));
        let exited = loop {
            let event = event_rx.recv_timeout(Duration::from_secs(5)).unwrap();
            if event.0 == "process:state" && event.1["state"] != "running" {
                break event.1;
            }
        };
        assert_eq!(exited["running_id"], running_id);
        assert!(matches!(
            exited["state"].as_str(),
            Some("exited") | Some("crashed")
        ));
        assert!(state.db.active_runs().unwrap().is_empty());
