...
async fn launch_instance_with_events(
    ...
    subs.insert("launcher_name", "enderloom".to_string());
    ...
    placeholders.insert("launcher_name", "enderloom".to_string());
    ...
    let started_at = now();
    process::spawn_process_with_events(
        events.clone(),
        &state.running,
        state.files.clone(),
        state.db.clone(),
    ...
    if let Some(activity) = crate::presence::activity_for(
        ...
        publish_pack_icon(state, instance, &settings, activity).await;
    }
...
mod tests {
    ...
    fn keeps_quoted_values_together() {
        assert_eq!(
            split_args("-Dname=\"Enderloom Launcher\" -Xmx4G"),
            vec!["-Dname=Enderloom Launcher", "-Xmx4G"]
        );
