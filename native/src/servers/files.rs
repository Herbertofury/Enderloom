...
pub fn read_text(files: &FileManager, dir: &Path, relative: &str) -> Result<ServerText> {
    ...
    if !kind.editable() {
        return Err(Error::other(
            "Enderloom cannot open this kind of file as text.",
        ));
    }
    ...
    let text = if kind == FileKind::Properties {
        ...
        String::from_utf8(bytes).map_err(|_| {
            Error::other("This file is not valid UTF-8, so Enderloom will not touch it.")
        })?
    };
    Ok(ServerText {
...
pub fn write_text(
    ...
    if !kind.editable() {
        return Err(Error::other(
            "Enderloom cannot save this kind of file as text.",
        ));
    }
