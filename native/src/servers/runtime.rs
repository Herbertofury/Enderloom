...
impl ServerEvents {
    pub fn tauri(app: &AppHandle) -> Self {
        Self::Tauri(app.clone())
    }

    pub fn ipc(sink: EventSink) -> Self {
        Self::Ipc(sink)
    }
    ...
    fn emit<T: Serialize>(&self, event: &str, payload: &T) {
        match self {
            ...
            Self::Ipc(sink) => match serde_json::to_value(payload) {
                ...
                Err(error) => tracing::warn!(%error, event, "could not serialize server event"),
            },
        }
    }
}
...
fn emit_state(events: &ServerEvents, info: ServerRunningInfo) {
    events.emit("server:state", &info);
}
                ...
                received = receiver.recv() => match received {
                    Some(line) => {
                        ...
                        if batch.len() >= EVENT_BATCH_LINES {
                            push_console(&events, &server_id, &console, std::mem::take(&mut batch));
                        }
                    }
                    None => {
                        push_console(&events, &server_id, &console, std::mem::take(&mut batch));
...
pub async fn start(
    ...
    start_with_events(ServerEvents::tauri(app), state, server).await
}
...
pub async fn start_ipc(
    ...
    start_with_events(ServerEvents::ipc(event_sink), state, server).await
}
...
async fn start_with_events(
    ...
    if script.is_none() && server.launch_jar.is_none() && server.launch_argfiles.is_empty() {
        ...
        let task = match &events {
            ServerEvents::Tauri(app) => {
                state
                    .tasks
                    .start(app, crate::tasks::TaskKind::ServerInstall, spec)?
            }
            ServerEvents::Ipc(_) => state
                .tasks
                .start_ipc(crate::tasks::TaskKind::ServerInstall, spec)?,
        };
    ...
    let control = match supervised {
        control::launch::Outcome::Supervised(session) => {
            ...
            spawn_console_flusher(events.clone(), server.id.clone(), console.clone(), receiver);
        ...
        control::launch::Outcome::Unavailable(reason) => {
            if script.is_some() {
                return Err(Error::other(format!(
                    "Enderloom could not supervise this pack's bootstrap script: {reason}"
                )));
            }
            ...
            let Some(process_started_at) = spawned_process_start(child_pid, &identity) else {
                ...
                return Err(Error::other(
                    "Enderloom could not verify the server process.",
                ));
            };
    ...
    emit_state(&events, info.clone());
    Ok(info)
}

fn finish_script_bootstrap(events: &ServerEvents, db: &crate::db::Db, server: &Server) {
    ...
    match rescan::run(db, server) {
        Ok(found) if found.launch_ready => {
            ...
            events.emit("server:rescanned", &json!({ "server_id": server.id }));
        }
        Ok(_) => {}
            ...
            while let Some(event) = reader.next().await {
                match event {
                    ...
                    control::Event::Process { pid, started_at } => {
                        ...
                        finish_script_bootstrap(&self.events, &self.db, &self.server);
...
pub async fn send_command(state: &AppState, server_id: &str, line: &str) -> Result<()> {
    let stdin = {
        ...
        match &handle.control {
            ...
            Control::Recovered { .. } => {
                return Err(Error::other(
                    "This server was started before Enderloom restarted, so commands cannot reach it. Restart it from Enderloom to get the console back.",
                ))
            }
        }
    };
...
pub async fn stop(app: &AppHandle, state: &AppState, server: &Server) -> Result<()> {
    stop_with_events(ServerEvents::tauri(app), state, server).await
}

pub async fn stop_ipc(event_sink: EventSink, state: &AppState, server: &Server) -> Result<()> {
    stop_with_events(ServerEvents::ipc(event_sink), state, server).await
}

async fn stop_with_events(events: ServerEvents, state: &AppState, server: &Server) -> Result<()> {
    let (stdin, recovered) = {
    ...
    let kill_deadline = tokio::time::Instant::now() + STOP_KILL_TIMEOUT;
    ...
    Err(Error::other(
        "The server process did not stop after Enderloom verified and terminated it.",
    ))
}
...
pub fn recover(app: &AppHandle, state: &AppState) -> Result<usize> {
    recover_with_events(ServerEvents::tauri(app), state)
}

pub fn recover_ipc(event_sink: EventSink, state: &AppState) -> Result<usize> {
    recover_with_events(ServerEvents::ipc(event_sink), state)
}

fn recover_with_events(events: ServerEvents, state: &AppState) -> Result<usize> {
    let mut recovered = 0;
    for run in state.db.active_server_runs()? {
        ...
        if let Some(control) = control::client::read_for(&state.paths.servers(), &server.id) {
            reconnect(events.clone(), state, &server, &run, control);
            recovered += 1;
            continue;
        }
...
fn reconnect(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let (sender, receiver) = mpsc::unbounded_channel();
        spawn_console_flusher(events.clone(), server_id.clone(), console.clone(), receiver);
        let watcher = Supervisor {
            events: events.clone(),
            db,
            registry,
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            if let Ok(tail) = files.read_tail_async(&path, offset).await {
                ...
                push_console(&events, &server_id, &console, batch);
            }
